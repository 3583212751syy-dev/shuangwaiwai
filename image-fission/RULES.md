...
- **🔴45 主体"拉开"只能靠换种子，不能靠压 cn**（用户第27轮："主体老鹰跟骷髅头的裂变需要跟原图拉开"）
  - 把 canny 压到 `cn .15 / dn 1.0` → **解剖必崩**：s1234 鹰头糊成黑块、s2024/s42 颅骨长黄斑（🔴35 红线）。
  - 正解 = 引导力度**回到已认可档** `cn .20 / end .55 / dn .98 @2048`，**靠换种子**逼出结构变化；
    选种判据 = 鹰头清晰可辨 + 黄喙在鹰脸上 + 颅骨无黄渍（黄像素占比 ≤ 原图 2.89%）。
    实测 seed 2024 入选（黄 1.62%）；s1234/s2025/s777/s91 鹰头不可辨 → 淘汰。
