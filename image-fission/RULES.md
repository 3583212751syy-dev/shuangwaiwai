...
| `pinterest6` | 鹰骷髅金属 + 尖刺标题 | `eagle_skull_canny` | `src/v390_p6canny.py` + `src/v401_p6parts.py` + `src/v403_words.py`；**主体档C 变化要"大" → `src/v411_skel.py`（🔴47 定稿）** | **档 C（自由重绘）**：整体弱 cn + 分部件通道 → 文本第二步（🔴37/41） |
...
- **🔴47 主体"变化大"的正解 = 几何造骨架 + 引导重画**（用户第 29 轮第 3 次指出"没拉开"后定稿）
  - **先量化"拉开没有"**：新增 **逐行宽度相关 `wcorr`**（每 y 行统计亮部像素数得轮廓宽度曲线，与原图求相关）——
    这就是轮廓同构度的直接度量。判据：`wcorr > 0.98` ⇒ **姿态没动**（无论 IoU 多低）。
    实测 v408 定稿 `wcorr 0.998`，用户的"没拉开"完全正确；目标 ≤0.95。
  - **三条路只有一条通**（第 29 轮全试过）：
    ❌ `v409`「掩膜内 denoise=1.0 自由重画」→ 姿态真变（wcorr .94~.97）但**质量塌**：
      多条区域提示词在 denoise=1.0 下互斗 → 鹰头糊、前额黄斑、胸前一团泥。
    ❌ `v410`「文生图重画 + 按位贴回」→ 变化最大（wcorr .55~.70）但**拼贴感**：
      新图自带一套射线、原图残射线对不上，接缝可见；且 `lum>55` 的自有掩膜会漏掉深棕鹰身
      （必须改"非黑即主体"的 `lum>22`）。
    ✅ `v411`「**几何造骨架 + 引导重画**」→ 兼顾。
  - **v411 定式**：① 几何上造新骨架（只提供"姿势"，不提供像素）——**核心带（鹰头+躯干，以体心为中线的
    `0.22×主体宽` 条带）不参与旋转**，只把核心带外侧的**翼板绕翼根**旋转（正角=PIL 逆时针；左翼 `-deg`、
    右翼 `+deg` ⇒ 双翼上举；反向 ⇒ 下掠），骷髅交给区域提示词；
    ② **骨架的 canny**（不是原图 canny）作 ControlNet `cn 0.42 / end 0.58`；
    ③ 掩膜内 `denoise 0.85` + v409 那套逐元素区域提示词（`region_scale 0.80`）；
    ④ `LatentUpscale` 到 2048 走自引导细节遍。
    认识论：**"改多少"靠结构，不该靠提示词；"画多好"靠重绘，不该靠贴图。**
  - ⚠️ **骨架踩坑**：按 x 中线硬切会把鹰头劈成两半各转各的（头中间留白块）→ 必须留"核心带"。
  - ⚠️ **hires 第二遍必须同样 `SetLatentNoiseMask`**（🔴46 只写了"自引导"，不够）：不加的话
    denoise 0.42 会把整幅裁块（含白射线与黑底）重画一遍 → **射线变乱絮、黑底糊出黄晕**。
  - **黑底保护升级 `sat_max`**：黑底区只放行「成片、低饱和的亮白（=新射线）」——
    `lum>100 且 max-min<55`，否则一律还原原图（拦掉自由重画糊出的淡黄/米色假光晕）。
  - **实测定稿**（v411，V1_wingV + `--wing 45`，seed 888）：sd 0.825（v408 0.940）/
    IoU 0.742（0.870）/ **wcorr 0.933（0.998）** / 黄 0.46% < 原图 0.76% /
    边缘密度 10.72 > 原图 9.68。变化是姿态级：横向大展翼 → **上举回收的高窄 V**、羽层重排、
    翼色偏金橄榄、颅骨收窄重画、下颌牙列重排、**射线保留原图不动**。
  - 载体：`src/v411_skel.py`（骨架+构建+两遍；`--only-sketch` 只出骨架可先目检）+
    `src/v409_p6free.py`（区域提示词库 VARIANTS / `_add_regions` / `_bg_protect` 复用）+
    `src/v409_eval.py`（含 wcorr 的评估）→ `v407_deliver.py` 走文本通道。
