...
REBIRTH_FILES = {
    ...
    # v390（用户第18轮否决"做的代码错乱就是不合格"+"不许碎片化"）：**定稿 = canny 结构重生 @2048**。
    # 三代教训：
    #   v373 canny(dn.90/cn.38/e.45 @2048) → 清晰，但结构锁太死 = "跟原图没区别"；
    #   v386 mode1 整幅 txt2img(880×1240) 被 4× 照片放大到 3543×4961 → 鹰糊成团 = "错乱"；
    #   v389 降 canny 试探：cn.18/.22 → 鹰头退化成色团（无逻辑，用户红线）。
    # 正解 = **保 2048 原生清晰度 + canny 只做构图骨架（cn.32/e.42）+ prompt 锁平涂硬边**。
    ...
    # v397（用户第22轮"主体生成错乱/要分步骤"）：**两通道主体裂变**（src/v397_p6head.py）
    #   A 通道 整体松引导(cn .20/.55/dn .98 @2560) → 翼/骷髅真换形；
    #   B 通道 鹰头 bbox 原生 1024² 中紧引导(cn .32/.50/dn .88) + 解剖 prompt
    #          （喙长在鹰脸上/眼可见/头转新角度）→ 治好 s888 的"白头噪点团+黄喙脱落"。
