"""
v285 — 回归 VAEEncodeForInpaint（v283 验证可靠），但给蝙蝠更大生成空间，同时锁死黑色材质。

修复 v283/v284 问题：
  1. 主体缺失/像素缺失 → 生成 mask 从蝙蝠剪影外扩 55px（v283 仅 40px），让翅膀完整展开
  2. 蝙蝠颜色漂移/变蓝 → IPAdapter 提到 0.18 强锁黑色/紫色材质，Canny 提到 0.95 强姿势约束
  3. 文字背景遮挡 → 沿用 v284 的局部中值填充弧带 + 均匀填充主副字，不做全图大模糊

架构：
  Stage B：局部中值去字（保留圆环/蝙蝠/内徽章）
  Stage A：VAEEncodeForInpaint + 外扩 55px 生成 mask + 严格蝙蝠剪影合成 mask
  Stage C：AbrilFatface 原位烧新字
"""
import argparse
import json
import math
...
SUBJECT_VARIANTS = [
    ("up",     "NOCTAVEN", "DISTILLERY", "SHADOW OF THE WING",  285001),
    ("spread", "DUSKBAT",  "RESERVE",    "WINGS OF TWILIGHT",   285101),
    ("fold",   "MOONBAT",  "NOCTURNE",   "GUARDIAN OF THE DARK", 285201),
]
...
NEG = (
    ...
    "green, brown, gray, beige, blue, cyan, violet, purple bat, colorful bat, "
    "color shift, blur, watercolor, photorealistic, "
    ...
    "anything other than pure black bat silhouette"
)
...
def make_text_free_source():
    """精确去字：弧带局部中值；主/副/Est/1862 均匀填充；不动圆环/蝙蝠/内徽章。"""
...
def make_generation_mask(size=(1552, 2000)):
    """VAEEncodeForInpaint 生成 mask：蝙蝠剪影外扩 55px，给翅膀留空间。"""
    w, h = size
    bat = np.array(make_bat_mask().convert("L")).astype(np.uint8)
    _, bat = cv2.threshold(bat, 80, 255, cv2.THRESH_BINARY)
    mask = cv2.dilate(bat, np.ones((55, 55), np.uint8), iterations=1)
    badge = np.zeros((h, w), np.uint8)
    cv2.circle(badge, (cx, cy), r_badge + 25, 255, -1)
    mask = cv2.bitwise_and(mask, badge)
...
def make_composition_mask(size=(1552, 2000)):
    """最终合成 mask：蝙蝠剪影 + 12px 缓冲，边缘羽化。"""
    w, h = size
    bat = np.array(make_bat_mask().convert("L")).astype(np.uint8)
    _, bat = cv2.threshold(bat, 80, 255, cv2.THRESH_BINARY)
    mask = cv2.dilate(bat, np.ones((12, 12), np.uint8), iterations=1)
...
def make_pose_guides():
    ...
    poses = {}
    up = _transform_silhouette(bat, sx=0.62, sy=1.50, rot_deg=-3)
    spread = _transform_silhouette(bat, sx=1.75, sy=0.58, rot_deg=0)
    fold_base = _transform_silhouette(bat, sx=1.10, sy=0.85, rot_deg=2)
    h, w = fold_base.shape
    ys = np.arange(h).reshape(-1, 1).astype(np.float32)
    factor = 1.0 + 0.32 * np.clip((ys - cy) / (h - cy), 0, 1) - 0.10 * np.clip((cy - ys) / cy, 0, 1)
    ...
    for tag, m in (("up", up), ("spread", spread), ("fold", fold)):
        ...
        path = COMFY_INPUT / f"v285_pose_{tag}.png"
        poses[tag].save(path, quality=95)
        print(f"[v285] pose guide {tag} -> {path}")
    return poses
...
def build(seed, tag, pose_img):
    ...
    pos = (
        f"pure solid black bat silhouette only, centered in dark purple circular badge, {bat_desc}, "
        f"NO text, NO letters, NO words, NO background change, "
        f"gothic emblem style, sharp clean edges, deep violet and black only, "
        f"the bat must be completely black, no blue, no purple on bat, no holes, no missing pixels"
    )

    g = {}
    g["1"] = {"class_type": "CheckpointLoaderSimple", "inputs": {"ckpt_name": CKPT}}
    g["2"] = {"class_type": "LoadImage", "inputs": {"image": "v285_text_free_source.png"}}
    g["3"] = {"class_type": "ImageScaleToTotalPixels", "inputs": {
        "image": ["2", 0], "upscale_method": "lanczos", "megapixels": 1.2, "resolution_steps": 64}}

    g["mask_load"] = {"class_type": "LoadImage", "inputs": {"image": "v285_generation_mask.png"}}
    ...
    g["20"] = {"class_type": "LoadImage", "inputs": {"image": f"v285_pose_{tag}.png"}}
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["12", 0], "filename_prefix": f"v285_{tag}"}}
    return g
...
def main():
    ...
    src_path = COMFY_INPUT / "v285_text_free_source.png"
    gen_mask_path = COMFY_INPUT / "v285_generation_mask.png"
    comp_mask_path = COMFY_INPUT / "v285_composition_mask.png"
    bat_mask_path = COMFY_INPUT / "v285_bat_mask.png"
    ...
    if not src_path.exists() or args.force:
        ...
        print(f"[v285] text-free source -> {src_path}")
        print(f"[v285] source check -> {JOB / '_source_check.png'}")
    else:
        print(f"[v285] reuse text-free source")

    if args.source_only:
        print("[v285] source-only mode, exit.")
        return

    if not bat_mask_path.exists() or args.force:
        bm = make_bat_mask()
        bm.save(bat_mask_path)
        print(f"[v285] bat mask -> {bat_mask_path}")
    else:
        print(f"[v285] reuse bat mask")
    ...
    if not gen_mask_path.exists() or args.force:
        ...
        print(f"[v285] generation mask -> {gen_mask_path}")
    else:
        print(f"[v285] reuse generation mask")
    ...
    if not comp_mask_path.exists() or args.force:
        ...
        print(f"[v285] composition mask -> {comp_mask_path}")
    else:
        print(f"[v285] reuse composition mask")
    ...
    for tag, big, sub, arc, seed in SUBJECT_VARIANTS:
        cands = sorted(COMFY_OUTPUT.glob(f"v285_{tag}_*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if not cands or args.force:
            print(f"[v285] generating {tag} ...")
            ...
            raw_path = cands[0]
            print(f"[v285] reuse {tag}: {raw_path.name}")
        ...
        composed.save(JOB / f"v285_{tag}_composed.png", quality=95)
        ...
        final_path = JOB / f"v285_{tag}_final.png"
