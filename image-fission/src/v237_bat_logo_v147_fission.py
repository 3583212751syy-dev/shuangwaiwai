...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v237-SDXL] submitted pid={pid[:8]} cfg={CFG} canny={CANNY_STRENGTH} (去蝙蝠Canny+新姿态)")
    for _ in range(120):
        time.sleep(5)
        try:
            h = history(pid)
...
def main():
    ...
    out_final = JOB / "v237_bat_logo.jpg"
    raw = gen(SEED)
    if not raw:
        print("[v237] FAIL gen")
        return
    ...
    tmp_path = JOB / "v237_sdxl_before_ocr.jpg"
