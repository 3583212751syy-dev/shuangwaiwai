...
SUBJECT_VARIANTS = [
    ("up",     "NOCTAVEN", "DISTILLERY", "SHADOW OF THE WING",      293001),
    ("spread", "DUSKBAT",  "RESERVE",    "WINGS OF TWILIGHT",       293101),
    ("fold",   "MOONBAT",  "NOCTURNE",   "GUARDIAN OF THE DARK",    293201),
]
...
def make_clean_source():
    ...
    # 0) 先在原图上找蝙蝠 mask，保护蝙蝠剪影
    bat_mask_pre = _detect_bat_mask(arr)
    print(f"[protect] bat_mask_pre sum={int(bat_mask_pre.sum()/255)}px")
    # 大幅扩张，确保覆盖 arc_band 内的翼尖（扩张 ~120px ~ ring 区域）
    big_kernel = np.ones((60, 60), np.uint8)
    bat_protect = cv2.dilate(bat_mask_pre, big_kernel, iterations=4) > 127
    print(f"[protect] bat mask {int(bat_mask_pre.sum()/255)}px -> protect {int(bat_protect.sum())}px")

    # 1) 拱门环带 = 整个环带的原图颜色 (包括文字)；环带之外全是紫底
    ...
    # 1a) 在环带内把非背景色像素全填成紫色，彻底去掉 BACARDÍ / LA CASA 字及描边
    in_band = arc_band_keep.copy()
    in_band[bat_protect] = False
    # 颜色与 bg_color 差异 > 阈值（非紫底/非蝙蝠保护区域）= 原图残留字/描边/噪点
    color_dist = np.linalg.norm(arr - bg_color, axis=2)
    non_bg = (color_dist > 35) & in_band
    clean[non_bg] = bg_color
    print(f"[arc] arc_band={int(arc_band.sum())} inner={int(inner.sum())} arc_band_keep={int(arc_band_keep.sum())} bat_protect={int(bat_protect.sum())} inter={int((arc_band_keep & bat_protect).sum())} in_band={int(in_band.sum())} non_bg_cleared={int(non_bg.sum())}")
    # debug vis
    debug_img = np.zeros((h, w, 3), dtype=np.uint8)
    debug_img[arc_band_keep] = (255, 0, 0)  # 红
    debug_img[bat_protect] = (0, 255, 0)  # 绿
    debug_img[arc_band_keep & bat_protect] = (255, 255, 0)  # 黄=交集
    Image.fromarray(debug_img).save(r"E:/Desktop/双接口/image-fission/jobs/v293/_debug_mask.png")
    ...
    # 3) 蝙蝠剪影保留
    bat_mask = _detect_bat_mask(clean)
    bat_pixels = clean.copy()
    bat_pixels[bat_mask == 0] = 0
    clean[bat_mask > 127] = bat_pixels[bat_mask > 127]
    print(f"[bat] {int((bat_mask>127).sum())}px")
...
def main():
    ...
    src_path = COMFY_INPUT / "v293_text_free_source.png"
    ...
    if not src_path.exists() or args.force:
        ...
        print(f"[v293] clean source -> {src_path}")
    else:
        print(f"[v293] reuse clean source")
    ...
    for tag, big, sub, arc, _ in SUBJECT_VARIANTS:
        ...
        composed.save(JOB / f"v293_{tag}_composed.png", quality=95)
        ...
        final_path = JOB / f"v293_{tag}_final.png"
        final.save(final_path, quality=95)
        files.append(final_path)
        print(f"[v293] {tag} final -> {final_path.name}")
    ...
    gp = JOB / "_grid_v293.png"
    grid.save(gp, quality=92)
    print(f"[v293] grid -> {gp}")
