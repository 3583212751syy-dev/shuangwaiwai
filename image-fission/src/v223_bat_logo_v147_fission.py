"""
v223 — bat_logo 真裂变（v221 同源，换 seed 隔离 edge 纹理变量）

v222 结论：lora 0.50→0.35 + 轻 USM 只把 edge 从 1.55× 降到 1.45×，
但清晰度从 116%→95% 明显退化 → edge 是 SDXL+upscaler 真裂变固有纹理，
后处理调参救不了。v223 = v221 全参数（lora 0.50 + 全 USM + 配色锁）换 seed 220002，
看不同 seed 的构图是否 edge 更低且仍锐，隔离 seed 变量。
...
SEED = 220002  # 新 seed，隔离 seed 变量（v223 是 220001）
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v223_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v223] submitted pid={pid[:8]} denoise={DENOISE} lora={LORA_DETAIL} (v220 + palette lock, same seed, controlled)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v223_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v223] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v223_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v223_bat_logo_raw.jpg"
        ...
        print(f"  saved v223_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v223.jpg"
