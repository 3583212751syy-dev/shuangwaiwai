"""
v246 — bat_logo strict original style（按原图 BACARDÍ bat logo 风格/设计/排版严格做）
...
SEED = 246001  # 与 v245 (245001)/v244 (244001)/v213 (213001) 不同
...
NEG_BASE = (
    ...
    # v246 新增 — 禁所有字形 (SDXL 不要画任何字母/字符/字形, 字由 PIL 后期烧)
...
GLOBAL_POS = (
    ...
    # 装饰描述 (SDXL 画装饰花纹, 不画字)
    "the top arched band contains only vintage fleur-de-lis and scrollwork ornamental pattern, "
...
REGIONS = [
    # 中心徽章: bat 旋转 + 徽章外环装饰
    {
        ...
        "prompt": (
            ...
            "the bat is rotated 30 degrees to its right and its wings tilted asymmetrically, "
            "the emblem has a black thin outline ring with vintage decorative scrollwork on the outside, "
            "no triangle above the bat, no column eyes, no eye symbols, no extra geometric symbols, "
    ...
    # 顶部弧形带: vintage fleur-de-lis + scrollwork (无任何字形)
    {
        "x": 0.10, "y": 0.02, "w": 0.80, "h": 0.18, "strength": 1.20,
        "prompt": (
            "a thin ornamental arched band at the top above the central emblem, "
            "filled with vintage fleur-de-lis and scrollwork decorative pattern ONLY, "
            ...
            "NO Roman letters, NO Greek letters, NO Cyrillic letters, "
            "NO necklace, NO pendant, NO ribbon, NO chain, NO draped fabric, "
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v246_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v246] submitted pid={pid[:8]} denoise={DENOISE} ipa={IPA_WEIGHT} (strict original style + vector + post-burn)")
    for _ in range(120):
        time.sleep(5)
        try:
            h = history(pid)
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v246_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v246] NOT FOUND raw_path")
        return None
    return raw_path
...
# ==================== v246 新增: PIL 后期烧非侵权词 (main 末尾, 不经过 SDXL 不会糊) ====================
...
def burn_top_arc(img, text, font_size, color=(26, 10, 31)):
    """烧顶部弧形带"""
    return draw_arc_text(
        img, text, FONT_PATH, font_size, color,
        (img.width // 2, int(img.height * 0.13)),
        int(img.width * 0.62), 100, 240, char_spacing_px=8, flip_180=False
    )
...
def main():
    out_final = JOB / "v246_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        try:
            ...
            print("  [v246] top arc burn OK: LVMEN NOCTIS")
        except Exception as e:
            print(f"  [v246] top arc burn FAIL: {e}")
        try:
            ...
            print("  [v246] bottom word burn OK: NOCTWING + MORS VINI")
        except Exception as e:
            print(f"  [v246] bottom word burn FAIL: {e}")
        try:
            burn_est_year(final_img, int(final_img.height * 0.022))
            print("  [v246] est year burn OK: Est. 1862")
        except Exception as e:
            print(f"  [v246] est year burn FAIL: {e}")
        ...
        print(f"  saved v246_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v246.jpg"
