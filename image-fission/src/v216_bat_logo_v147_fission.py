"""
v216 — bat_logo 真裂变（v215 + 修残留）

v215 已是最接近 v147 的真裂变，但残留：
  1. 底部黑色水滴吊坠（v214 残留）
  2. 构图略偏左中
  3. edge_density 1.55x 超 1.3 阈值（卷草花纹太多）

v216 修复：
  - denoise 0.80 → 0.70（保留更多原图徽章结构，少产生新花纹）
  - LORA_DETAIL 1.0 → 0.65（减少细节生成 → 降 edge）
  - NEG 加 pendant / teardrop / hanging gem / drop shape / charm / off-center / asymmetric
  - POS 加 perfectly centered + symmetrical composition
  - 后处理：color_transfer 后加轻 USM 锐化主轮廓
技术栈：ComfyUI + ProteusV0.4 + IPAdapter Plus + Canny 0.25 + Tile 0.60
        + add-detail LoRA(0.65) + Reinhard LAB 配色迁移
"""
...
SEED = 216001  # 与 v213/214/215 都不同
...
# ==================== NEG：v215 + v216 强化（禁 starburst + 禁可读字 + 禁写实 + 禁珠宝+吊坠+偏中心） ====================
NEG_BASE = (
...
# ==================== bat_logo 参考图（与原图同源: BACARDÍ bat badge）====================
REF_IMG = "test_6978fabda2cc99629fa9e81f802762d3.jpg"

# ==================== Prompt 框架（v215 + v216 居中对称强化）====================
GLOBAL_POS = (
    ...
    "a 2D flat vintage circular badge with an evolved vector silhouette bat in the center, "
    "the bat is a stylized 2D silhouette wing-only graphic (NOT realistic, NOT 3D, NOT photographic), "
    "decorative ornamental CURVES and curlicues woven into the badge as purely graphical motifs "
    "(abstract letterform-shaped flourishes and baroque scrollwork, NOT forming any readable word, "
    "NOT spelling anything, NOT any brand name), "
    "the emblem ring is rotated 18 degrees and slightly distorted with baroque flourishes, "
    "small geometric crescent and triangle accents around the badge ring, "
    "vintage craft spirits label art style, sharp clean edges, bold emblem, "
    "flat 2D illustration style, symmetric composition, NO starburst, NO exploding star, "
    "NO firework, NO photographic, NO jewelry, NO pendant, NO teardrop, NO hanging gem"
)

REGIONS = [
    # 中心徽章：保留徽章骨架 + 剪影蝙蝠重画 + 居中
    {
        "x": 0.05, "y": 0.10, "w": 0.90, "h": 0.80, "strength": 1.30,
        "prompt": (
            "a perfectly centered flat 2D vintage circular purple badge with an evolved vector silhouette bat inside, "
            "the bat is a stylized 2D wing-spread silhouette in pure black (NOT photographic, NOT 3D, NOT realistic), "
            "two bats instead of one (a main large bat and a smaller secondary bat flying above-right), "
            "decorative ornamental CURVES and curlicues woven into the lower curve of the badge "
            "as purely abstract graphical motifs (NOT readable letters, NOT spelling any word, "
            "NOT any brand name, looks like artistic baroque scrollwork integrated into the design), "
            "a thin ornamental crescent moon replaces the original moon, "
            "the emblem ring is twisted slightly with baroque flourishes and 5 ornamental hanging beads, "
            "the bottom now has a stylized laurel wreath accent replacing the original triangle, "
            "keep purple/black/white palette and craft spirits label style, flat 2D illustration, "
            "perfectly centered and symmetrical. " + COHESIVE
        )
    },
    # 顶部弧形带：装饰性图案带
    {
        ...
        "prompt": (
            ...
            "purple and black color blocks with sharp edges and baroque flourishes, perfectly centered. " + COHESIVE
        )
    },
    # 底部徽章饰带：装饰性图案
    {
        ...
        "prompt": (
            "a flat 2D bottom decorative banner with abstract ornamental patterns and stylized "
            "curves as decorative motifs (NOT readable English words, NOT spelled-out words, "
            "NOT starburst, NOT jewelry, NOT pendant, NOT teardrop), "
            "the motifs look like artistic baroque scrollwork that complement the design, "
            "kept purple and black palette, flat 2D vintage style, perfectly centered. " + COHESIVE
        )
    },
]
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v216_bat_logo"}}
    return g
...
def unsharp(rgb_img, radius=1.5, percent=60, threshold=2):
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v216] submitted pid={pid[:8]} denoise={DENOISE} lora={LORA_DETAIL} (v215 skeleton + centered + less texture)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v216_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v216] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v216_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        # v216 后处理：轻 USM 锐化主轮廓（降"补丁感"，不显著增加 edge）
        matched_rgb = Image.fromarray(cv2.cvtColor(matched, cv2.COLOR_BGR2RGB))
        matched_rgb = unsharp(matched_rgb, radius=1.5, percent=60, threshold=2)
        out_raw = JOB / "v216_bat_logo_raw.jpg"
        ...
        print(f"  saved v216_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v216.jpg"
