"""
v319: Fix v318c's two complaints
  1. text too small/different style  -> switch Anton (condensed) to AbrilFatface (Didone serif,
     wider w/capH ratio, matches original ARMED/FORCES style with serif bars and triangular A/D)
  2. background camo blurred beside text -> shrink mask dilate from 13x13 ellipse to 4x4 (tight
     to letter outline) so LaMa only touches the exact text region, not the surrounding camo

AprilFatface metrics @ fsize=140: capH=98, "BRAVE" w=475 (vs original ARMED 1322).
Combined with letter spacing 200-260px gives total width ~1300px matching original.
"""
import cv2
import numpy as np
import shutil
...
# AbrilFatface h/font_size ≈ 0.72 (cap height).
# To get capH=99 (original big1/big2 band text height), fsize = 99/0.72 ≈ 138.
# Use 140 for slight overflow (military bold look).
FSIZE = 140

# Letter spacing (px between characters) to match original text width.
# Original widths: small 517, big1 1322, big2 917.
# AbrilFatface @ 140 widths: WE_SUPPORT_THE ~?, HEROES 570, BRAVE 475, LEGION 521
# (compute dynamically below).
# Use spacing to land total_w near orig_w:
#   small: 14 chars (incl spaces)  -> ~(517-1225)/13 = -54  (already too wide, use 0)
#   big1:  5 chars                  -> (1322-475)/4 = 212
#   big2:  6 chars                  -> (917-521)/5  = 79
BANDS = [
    {'name': 'small', 'y0': 472, 'y1': 523, 'new_text': 'HONOR', 'orig_cx': 783, 'spacing': 0},
    {'name': 'big1',  'y0': 541, 'y1': 640, 'new_text': 'BRAVE', 'orig_cx': 829, 'spacing': 200},
    {'name': 'big2',  'y0': 656, 'y1': 755, 'new_text': 'LEGION', 'orig_cx': 863, 'spacing': 70},
]
...
# Step 2: TIGHT mask dilate (4x4 ellipse) - was 13x13 in v318c which blurred surroundings
...
print(f'[v319] text mask px: {int(text_mask_d.sum())} (was 168661 in v316e with 13x13 dilate)')

mask_pil = Image.fromarray((text_mask_d.astype(np.uint8) * 255), mode='L')
src_pil = Image.fromarray(img_rgb)
print('[v319] Running LaMa inpaint with TIGHT mask...')
...
# Step 3: write text with AbrilFatface Didone serif, sized to match original
out_pil = cleaned.copy()
draw = ImageDraw.Draw(out_pil)

# Verify AbrilFatface metrics
font = ImageFont.truetype(ABRIL_PATH, FSIZE)
for band in BANDS:
    text = band['new_text']
    bbox = font.getbbox(text)
    nat_w = bbox[2] - bbox[0]
    nat_h = bbox[3] - bbox[1]
    print(f'[v319-font] "{text}" nat_w={nat_w} nat_h={nat_h} capH/fontSize={nat_h/FSIZE:.2f}')
...
for band in BANDS:
    ...
    paste_y = band['y1'] - nat_h - bbox[1]  # align bottom of text to band bottom (matches original)
    draw.text((paste_x, paste_y), text, font=font, anchor='lt',
              fill=(20, 20, 20), spacing=spacing)
    print(f'[v319] {band["name"]}: "{text}" total_w={total_w} paste=({paste_x},{paste_y})')

OUT = Path(OUT_DIR) / 'armed_text_only_v319.jpg'
...
combined.save(Path(OUT_DIR) / 'armed_compare_v319.jpg', quality=90)
...
band_compare.save(Path(OUT_DIR) / 'armed_text_band_compare_v319.jpg', quality=95)
print(f'[v319] Saved {OUT}')
