...
SUBJECT_VARIANTS = [
    ("up",     "NOCTAVEN", "DISTILLERY", "SHADOW OF THE WING",      295001),
    ("spread", "DUSKBAT",  "RESERVE",    "WINGS OF TWILIGHT",       295101),
    ("fold",   "MOONBAT",  "NOCTURNE",   "GUARDIAN OF THE DARK",    295201),
]
...
def make_clean_source():
    ...
    Image.fromarray(debug_img).save(r"E:/Desktop/双接口/image-fission/jobs/v295/_debug_mask.png")
...
def make_variants(src_img):
    ...
    variants["spread"] = _warp_bat_by_wings(bat_pixels, mask,
        left_delta=(-30, 0), right_delta=(30, 0),
        shoulder_l=lm["left_shoulder"], shoulder_r=lm["right_shoulder"], falloff=110)
...
def main():
    ...
    src_path = COMFY_INPUT / "v295_text_free_source.png"
    ...
    if not src_path.exists() or args.force:
        ...
        print(f"[v295] clean source -> {src_path}")
    else:
        print(f"[v295] reuse clean source")
    ...
    for tag, big, sub, arc, _ in SUBJECT_VARIANTS:
        ...
        composed.save(JOB / f"v295_{tag}_composed.png", quality=95)
        ...
        final_path = JOB / f"v295_{tag}_final.png"
        final.save(final_path, quality=95)
        files.append(final_path)
        print(f"[v295] {tag} final -> {final_path.name}")
    ...
    gp = JOB / "_grid_v295.png"
    grid.save(gp, quality=92)
    print(f"[v295] grid -> {gp}")
