"""
v227 — bat_logo 真裂变（v226 同源，动态俯冲姿态变体）

v226 已达成首个干净 QC PASS（edge 1.12× / laplacian 1.06× / 字体 1.22×）。
v227 同 seed 220001 受控，仅改主体蝙蝠姿态（v147 允许"改主体角度/姿态"）：
  - 蝙蝠从标准展翅 → "wings swept upward, head lowered diving pose"（更具动感哥特俯冲姿态）
  - 保留 v226 全部：单只 / 细净环线 / 无弯月 / 无悬挂物 / 配色锁 / NMKD 4x
目标：给用户"标准展翅(v226)" vs "动态俯冲(v227)"的视觉选择，同时维持 edge<1.3× 干净 PASS。
"""
...
GLOBAL_POS = (
    ...
    "the bat is a single stylized 2D silhouette in pure black with wings swept upward and head lowered in a dynamic diving gothic pose (NOT realistic, NOT 3D, NOT photographic), "
...
REGIONS = [
    # 中心徽章：单只大胆剪影蝙蝠 + 干净巴洛克框（去 two-bats / 去 hanging beads）
    {
        ...
        "prompt": (
            ...
            "the bat is a single stylized 2D silhouette in pure black with wings swept upward and head lowered in a dynamic diving gothic pose (NOT photographic, NOT 3D, NOT realistic), "
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v227_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v227] submitted pid={pid[:8]} denoise={DENOISE} lora={LORA_DETAIL} (v221 + cleaner composition, same seed, controlled)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v227_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v227] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v227_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v227_bat_logo_raw.jpg"
        ...
        print(f"  saved v227_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v227.jpg"
