...
GLOBAL_POS = (
    ...
    "the bat is shown from a dynamic three-quarter view with its wings spread in a WIDE upward V and ROTATED about 25 degrees to its left, "
    "head tilted to the RIGHT and looking upward, one wing slightly higher than the other - a clearly DIFFERENT but related pose from the flat front-facing source bat, "
    "but unmistakably the SAME heraldic bat species and clearly related to the source bat, NOT inverted, NOT drooping, NOT a copy of the source pose, "
...
REGIONS = [
    ...
    "the bat is in a NEW dynamic pose: wings spread in a WIDE upward V, body rotated about 25 degrees, head tilted to the side and looking up - "
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v239_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v239-SDXL] submitted pid={pid[:8]} cfg={CFG} canny={CANNY_STRENGTH} (去蝙蝠Canny+宽V左旋姿态)")
    for _ in range(120):
        time.sleep(5)
        try:
            h = history(pid)
...
def main():
    ...
    out_final = JOB / "v239_bat_logo.jpg"
    raw = gen(SEED)
    if not raw:
        print("[v239] FAIL gen")
        return
    ...
    tmp_path = JOB / "v239_sdxl_before_ocr.jpg"
    ...
    print(f"[v239] erased {n_erased} char cluster(s)")

    out_cmp = JOB / "_compare_v239.jpg"
