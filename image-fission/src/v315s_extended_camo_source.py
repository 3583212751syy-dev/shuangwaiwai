...
# Step 2: per-band per-pixel random camo sampling with EXTENDED source
# Sample from band + 50px padding above and below (text-free zone)
PAD = 50
for band in BANDS:
    y0, y1 = band['y0'], band['y1']
    band_h = y1 - y0
    ...
    # Camo source = extended region: band ± PAD, minus text and chain
    src_y0 = max(0, y0 - PAD)
    src_y1 = min(H, y1 + PAD)
    src_rgb = img_rgb[src_y0:src_y1, :]
    src_gray = gray[src_y0:src_y1, :]
    src_chain = chain_mask[src_y0:src_y1, :] > 0
    # In the extended region, text is only within the band y range
    src_text = np.zeros_like(src_gray, dtype=bool)
    text_start_in_src = max(0, y0 - src_y0)
    text_end_in_src = text_start_in_src + band_h
    src_text[text_start_in_src:text_end_in_src, :] = (src_gray[text_start_in_src:text_end_in_src, :] < 180)
    camo_mask = ~src_text & ~src_chain
    camo_pixels = src_rgb[camo_mask]  # (N, 3)
    ...
    print(f'[v315s] {band["name"]}: filled {n_text} text px, camo source {len(camo_pixels)} px (band ± {PAD}px)')
