"""v120：IPAdapter 强度 0.30/0.45 × 剩余 5 张（用户选档验证）
用户反馈（2026-08-27）：「按照0.3跟0.45做给我看看」
- 参数同 v118 已验证管线，仅 IPAdapter weight 改为 0.30 / 0.45
- 5 张：eagle_2 / denim_3 / camo_4 / skull_5 / metal_6（illust_1 已有 w030/w045 在 slider）
- 每张各保原图比例，denoise 0.72

用法：python src/smoke_v120_weights.py
"""
...
REFS = [
    {
        ...
        "pos": ("t-shirt graphic design, majestic spread-winged eagle gripping a skull in its talons, "
                "gothic crest emblem with chains and flames, same composition and layout as reference image, "
                "dark black background, white and red linework, high contrast, "
                "REDESIGNED internal details, new intricate feather patterns, beautifully reimagined crest ornaments, "
        ...
        "neg": ("no gray, no midtones, no gradient, no soft shading, no halftone, "
    ...
    },
    {
        ...
        "pos": ("t-shirt graphic design, single delicate moth flying over a distressed X-shaped denim patch, "
                "same composition and layout as reference image, "
                "vintage faded denim texture, indigo blue and white only, rough brushstroke X mark, "
                "fabric wear and tear, dotted flight trail, high contrast, "
                "REDESIGNED internal details, new intricate denim stitches, beautifully reimagined moth wing patterns, "
                "masterpiece, best quality, ultra detailed, clean vector illustration, sharp clean edges, "
                "fills entire frame edge-to-edge, no margins, full bleed, no halftone, no noise, no grain"),
        "neg": ("no gray, no midtones, no gradient, no soft shading, no halftone, "
                "3d, photographic, painterly, blur, noise, grain, pixelated, jagged edges, "
    ...
    },
    {
        ...
        "pos": ("t-shirt graphic design, tropical palm tree silhouette with classic military camouflage pattern, "
                "same composition and layout as reference image, "
                "tropical jungle camouflage, brown green tan khaki black palette, "
                "black palm tree silhouettes, organic camo blob shapes, high contrast, "
                "REDESIGNED internal details, new intricate palm leaf textures, beautifully reimagined camo patterns, "
                "masterpiece, best quality, ultra detailed, clean vector illustration, sharp clean edges, "
                "fills entire frame edge-to-edge, no margins, full bleed, no halftone, no noise, no grain"),
        "neg": ("no gray, no midtones, no gradient, no soft shading, no halftone, "
                "3d, photographic, painterly, blur, noise, grain, pixelated, jagged edges, "
    ...
    },
    {
        ...
        "pos": ("t-shirt graphic design, grim human skull with red snake coiled around it and red angel wings, "
                "same composition and layout as reference image, "
                "gothic tattoo illustration, pure black background, red and white palette, "
                "blood drips, cracked bone, serpent scales, feathered wings, high contrast, "
                "REDESIGNED internal details, new intricate serpent scale patterns, beautifully reimagined wing feathers, "
                "masterpiece, best quality, ultra detailed, clean vector illustration, sharp clean edges, "
                "fills entire frame edge-to-edge, no margins, full bleed, no halftone, no noise, no grain"),
        "neg": ("no gray, no midtones, no gradient, no soft shading, no halftone, "
                "3d, photographic, painterly, blur, noise, grain, pixelated, jagged edges, "
    ...
    },
    {
        ...
        "pos": ("t-shirt graphic design, bald eagle perched on a cracked human skull with horn-like spikes and lightning bolts, "
                "same composition and layout as reference image, "
                "death metal thrash metal illustration, pure black background, white and bronze gold yellow palette, "
                "sharp aggressive lines, radial lightning burst, heavy ink, high contrast, "
                "REDESIGNED internal details, new intricate skull cracks, beautifully reimagined lightning patterns, "
                "masterpiece, best quality, ultra detailed, clean vector illustration, sharp clean edges, "
                "fills entire frame edge-to-edge, no margins, full bleed, no halftone, no noise, no grain"),
        "neg": ("no gray, no midtones, no gradient, no soft shading, no halftone, "
                "3d, photographic, painterly, blur, noise, grain, pixelated, jagged edges, "
...
def build(ref, seed, weight):
    ...
    g["14"] = {"class_type": "SaveImage",
               "inputs": {"images": ["13", 0],
                          "filename_prefix": f"v120_{ref['id']}_w{int(weight*100):03d}"}}
    return g


def run(ref, seed, weight, out_base):
    tag = f"{ref['id']}_w{int(weight*100):03d}"
    r = requests.post(f"{COMFYUI}/prompt",
                      json={"prompt": build(ref, seed, weight),
                            "client_id": f"v120_{int(time.time())}"}, timeout=10)
    if r.status_code != 200:
        print(f"[ERR] {tag} submit: {r.status_code}", flush=True); return False
...
def main():
    out = PROJECT_ROOT / "jobs" / f"smoke_v120_{int(time.time())}"
    out.mkdir(parents=True, exist_ok=True)
    print(f"=== v120 IPAdapter 0.30/0.45 × 5 张 ===", flush=True)
    seed = 200301
    for w in WEIGHTS:
        for ref in REFS:
            run(ref, seed, w, out)
