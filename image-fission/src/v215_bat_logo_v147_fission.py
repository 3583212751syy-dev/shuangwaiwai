"""
v215 — bat_logo 真裂变（v213 骨架 + v214 NEG + 强调剪影蝙蝠）

v213 违和：上方炸裂星形 + 中央"TMG MME CENT"含义不明
v214 矫枉过正：徽章消失变成 3D 项链吊坠
v215 思路：
  - 保留 v213 的徽章骨架（圆环 + 蝙蝠 + 弧字位置 + 装饰结构）
  - 强化 v214 的 NEG（禁 starburst/firework/photographic bat/no readable word）
  - 强调主体剪影风格（vector silhouette, 2D flat, NOT 3D, NOT photographic）
  - 中央文字改为纯装饰花纹（彻底解决"含义不明"问题）

技术栈：ComfyUI + ProteusV0.4 + IPAdapter Plus + Canny 0.25 + Tile 0.60
        + add-detail LoRA + Reinhard LAB 配色迁移（同 v213/v214 基线/v147 风格）
"""
...
SEED = 215001  # 与 v213 (213001)、v214 (214001) 都不同
...
# ==================== NEG：v214 强化版（禁 starburst + 禁可读字 + 禁写实 + 强保剪影） ====================
NEG_BASE = (
    ...
    "jewelry, necklace, pendant, gem, gemstone, jewelry setting, earring, ring, brooch, "
...
# ==================== Prompt 框架（v213 骨架 + v214 强化 + 强调剪影蝙蝠）====================
GLOBAL_POS = (
    ...
    "flat 2D illustration style, NO starburst, NO exploding star, NO firework, NO photographic, NO jewelry"
)
...
REGIONS = [
    # 中心徽章：保留徽章骨架 + 剪影蝙蝠重画
    {
        ...
        "prompt": (
            "a flat 2D vintage circular purple badge with an evolved vector silhouette bat inside, "
            ...
            "keep purple/black/white palette and craft spirits label style, flat 2D illustration. " + COHESIVE
        )
    },
    # 顶部弧形带：装饰性图案带
    {
    ...
    # 底部徽章饰带：装饰性图案
    {
        ...
        "prompt": (
            ...
            "NOT starburst, NOT jewelry, NOT pendant), "
            "the motifs look like artistic baroque scrollwork that complement the design, "
            "kept purple and black palette, flat 2D vintage style. " + COHESIVE
        )
    },
]
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v215_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v215] submitted pid={pid[:8]} denoise={DENOISE} ipa={IPA_WEIGHT} (v213 skeleton + v214 NEG + vector-bat-emphasis)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v215_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v215] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v215_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v215_bat_logo_raw.jpg"
        ...
        print(f"  saved v215_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v215.jpg"
