"""
v318b: v318 + improved text layout. Use original text X center, modest
horizontal scale (sx=1.6-2.0) so new text reads as a military stencil
banner instead of a tiny narrow strip. LaMa fill unchanged.
"""
import cv2
import numpy as np
import shutil
...
def measure_band_x_extent(gray, y0, y1):
    band = gray[y0:y1, :]
    dark = (band < 80).astype(np.uint8)
    col = dark.sum(axis=0)
    in_col = col > 5
    if not in_col.any():
        return None, None
    left = int(np.where(in_col)[0][0])
    right = int(np.where(in_col)[0][-1])
    return left, right


# BANDS: original measured X centers + band Y; target text + sx
# small cx=783 orig_w=517; big1 cx=829 orig_w=1322; big2 cx=863 orig_w=917
BANDS = [
    {'name': 'small', 'y0': 472, 'y1': 523, 'new_text': 'HEROES',     'orig_cx': 783, 'sx': 1.6},
    {'name': 'big1',  'y0': 541, 'y1': 640, 'new_text': 'BRAVE',      'orig_cx': 829, 'sx': 2.0},
    {'name': 'big2',  'y0': 656, 'y1': 755, 'new_text': 'LEGION',     'orig_cx': 863, 'sx': 1.8},
]
...
# Step 2: dilate to capture anti-aliased edges, exclude chain
...
print(f'[v318b] text mask px: {int(text_mask_d.sum())}')

# Step 3: LaMa inpaint
src_pil = Image.fromarray(img_rgb)
print('[v318b] Running LaMa inpaint...')
...
# Step 4: write new text — Anton, scaled, X-centered on original text center
...
for band in BANDS:
    ...
    new_w = int(nat_w * sx)
    new_h = int(nat_h)
    # Build text layer (RGBA for clean paste) then horizontal scale
    ...
    txt_scaled = txt_layer.resize((new_w, new_h), Image.LANCZOS)
    # X center to original text center
    paste_x = band['orig_cx'] - new_w // 2
    paste_y = (band['y0'] + band['y1']) // 2 - new_h // 2
    out_pil.paste(txt_scaled, (paste_x, paste_y), txt_scaled)
    print(f'[v318b] {band["name"]}: "{text}" fsize={fsize} nat={nat_w}x{nat_h} -> {new_w}x{new_h} sx={sx} paste=({paste_x},{paste_y})')

OUT = Path(OUT_DIR) / 'armed_text_only_v318b.jpg'
...
combined.save(Path(OUT_DIR) / 'armed_compare_v318b.jpg', quality=90)
...
band_compare.save(Path(OUT_DIR) / 'armed_text_band_compare_v318b.jpg', quality=95)
print(f'[v318b] Saved {OUT}')
