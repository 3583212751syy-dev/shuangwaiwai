...
SUBJECT_VARIANTS = [
    ("up",     "NOCTAVEN", "DISTILLERY", "SHADOW OF THE WING",      294001),
    ("spread", "DUSKBAT",  "RESERVE",    "WINGS OF TWILIGHT",       294101),
    ("fold",   "MOONBAT",  "NOCTURNE",   "GUARDIAN OF THE DARK",    294201),
]
...
def make_clean_source():
    ...
    # 1a) 在环带内（除蝙蝠保护区外）全部填成紫底，彻底去掉 LA CASA / BACARDÍ 字及所有描边
    in_band = arc_band_keep.copy()
    in_band[bat_protect] = False
    clean[in_band] = bg_color
    print(f"[arc] arc_band={int(arc_band.sum())} inner={int(inner.sum())} arc_band_keep={int(arc_band_keep.sum())} bat_protect={int(bat_protect.sum())} inter={int((arc_band_keep & bat_protect).sum())} in_band={int(in_band.sum())} cleared={int(in_band.sum())}")
    # debug vis
    ...
    Image.fromarray(debug_img).save(r"E:/Desktop/双接口/image-fission/jobs/v294/_debug_mask.png")
...
def main():
    ...
    src_path = COMFY_INPUT / "v294_text_free_source.png"
    ...
    if not src_path.exists() or args.force:
        ...
        print(f"[v294] clean source -> {src_path}")
    else:
        print(f"[v294] reuse clean source")
    ...
    for tag, big, sub, arc, _ in SUBJECT_VARIANTS:
        ...
        composed.save(JOB / f"v294_{tag}_composed.png", quality=95)
        ...
        final_path = JOB / f"v294_{tag}_final.png"
        final.save(final_path, quality=95)
        files.append(final_path)
        print(f"[v294] {tag} final -> {final_path.name}")
    ...
    gp = JOB / "_grid_v294.png"
    grid.save(gp, quality=92)
    print(f"[v294] grid -> {gp}")
