"""
v233 — v232 baseline + OCR 已知字符崩坏根治

OCR 客观数据 v232:
  ❌ beamsearch ['7', 'HAGARIA', '(', 'S1UIEZI']
  ❌ greedy     ['7', 'HAGARIA', '(', 'S1UIEZ I7']

根因：SDXL 看到圆环 + arched band 必然想加"装饰性 lettering"
v232 NEG 里的"legible text, readable text, pseudo letters"全部 fail
因为 NEG 是英文描述，SDXL CLIP encoder 不一定精确权重 (尤其自造的 pseudo lettering 不是 readable)

v233 根治策略（更激进）：
  1. 顶部 arched band 简化为一条纯细线（decoration 降最少）
  2. 中央徽章内禁止任何 horizontal-oriented glyph 形状生成
  3. NEG 大幅扩张，把 OCR 已知坏字符加入(防 hash)
  4. 加入 "no horizontal text-like shapes anywhere"
  5. cfg 从 6.5 提到 8.0 让 NEG 权重更被听
"""
...
CFG = 8.0  # v232 是 6.5，提升让 NEG 更被听

# v232 已实测崩坏字符（OCR 实证，禁止 hash）
NEG_BASE = (
    "frame, border, white border, edge border, box outline, padding, margin, empty corners, letterbox, "
    # —— 已知崩坏字符（OCR 实证，v232 直接列出）——
    "text, letters, words, characters, alphabet, alphabet letters, "
    ...
    "any individual character shapes that resemble HAGARIA, S1UIEZI, GALLERIE, SDRUEIAMM, BACLRET, SMIRAICE, TMG, MME, CENT, NOCTURNE, "
    "BACARDI, Mooncrest, Curse, "
    # —— 3D / 茶器 / 瓷器 ——
...
# v233 prompt 极致收紧：完全不要任何 letterform ribbons 描绘
GLOBAL_POS = (
    ...
    "the ring is a CLEAN THIN LINE outline with NOTHING on it - no decoration, no ornamentation, no letters, no glyphs, no scribbles, "
    "the area ABOVE the badge is COMPLETELY EMPTY smooth purple background, no arched band, no banner, no ribbon, no curved line, no decoration, "
    "the area BELOW the badge is COMPLETELY EMPTY smooth purple background, no banner, no ribbon, no triangle, no decoration, "
    ...
    "the ONLY shapes present are: a thin ring, a stylized bat, and small abstract decorative geometric ornaments shaped like circles, dots, or tiny triangles (NOT letters)"
)
...
REGIONS = [
    {
        ...
        "prompt": (
            "the CENTER circular emblem with a thin clean ring outline ONLY as the same abstract frame, "
            ...
            "ring is a clean thin LINE outline, NOTHING on the ring line, NO letters, NO glyphs, NO scribbles, "
    ...
    {
        "x": 0.10, "y": 0.00, "w": 0.80, "h": 0.15, "strength": 1.20,
        "prompt": (
            "completely EMPTY smooth saturated purple space above the badge, "
            "NO arched band, NO curved line, NO banner, NO ribbon, NO scroll, NO ring on top, NO border band, "
            "NO letters, NO text, NO words, NO pseudo-letters, NO letter fragments, NO logo, NO monogram, NO glyphs, NO random characters, NO marks, NO squiggles, NO scribbles, "
            "NO decoration, NO wreath, NO laurel, NO filigree, NO crescent, NO star, NO dot pattern, NO line pattern, "
    ...
    },
    {
        ...
        "prompt": (
            "completely EMPTY smooth saturated purple space below the badge, "
            "NO banner, NO ribbon, NO scroll, NO pendant, NO triangle, NO drop shape, NO laurel, NO crescent, NO star, NO dot, "
            "NO letters, NO words, NO logos, NO gems, NO floating object, NO monogram, NO glyphs, NO marks, NO random characters, NO scribbles, "
            "STRICTLY solid saturated purple background, perfectly centered. " + COHESIVE
        )
    },
]
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v233_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v233] submitted pid={pid[:8]} cfg={CFG} canny={CANNY_STRENGTH} (v232 + OCR字符禁词) ")
    for _ in range(120):
        time.sleep(5)
        try:
            h = history(pid)
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v233_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v233] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v233_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v233_bat_logo_raw.jpg"
        ...
        print(f"  saved v233_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    out_cmp = JOB / "_compare_v233.jpg"
