"""
v249 — bat_logo strict original style（按原图 BACARDÍ bat logo 风格/设计/排版严格做）
...
原图精确风格/设计/排版:
  ...
  - 排版: 上中下竖向堆叠, 居中对称
v249 改造 (收敛"装饰过度繁复"反复反馈):
  1. v147 锁基线 (denoise 0.80 / ipa 0.18 / lora 1.0 / canny 0.25 / tile 0.60)
  2. 严格按原图极简风: 浅紫背景 + 三层布局 + bat 居中展翅, 整体做减法
  3. bat 旋转 30 度 + 翅膀倾斜 (v242/v244 被打脸"跟原图没区别", 必须真改)
  4. 顶弧 + 底部饰带均 SDXL 完全留白 (strength 0.40), 装饰花纹/字全 PIL 后期烧 (顶弧 LVMEN NOCTIS / 底部 NOCTWING+MORS VINI / Est.1862)
  5. 徽章外环做减法: simple thin ring, 禁 scrollwork/fleur-de-lis/ornate filigree/罗盘表盘
  6. GLOBAL_POS + NEG 双保险禁过度装饰/繁复/baroque/scrollwork
  7. 强拉 2D vector cartoon sticker papercut (修 v245 "flat 2D printed" 联想 3D 浮雕)
  8. Reinhard LAB 锁色 (保持原图浅紫 + 深紫 + 黑)
  9. AI 视觉盲无法肉眼自检, 客观 QC + 用户目检判定
...
SEED = 249001  # 与 v248 (248001)/v246 (246001)/v245 (245001)/v244 (244001)/v213 (213001) 不同
...
NEG_BASE = (
    ...
    # v249 新增 — 禁所有字形 (SDXL 不要画任何字母/字符/字形, 字由 PIL 后期烧)
    ...
    # v249 新增 — 禁罗盘/表盘/指针/时钟类崩坏 (v247 暴露徽章底部"罗盘/表盘")
    ...
    # v249 新增 — 禁过度装饰/繁复 (v246/v247 反复"装饰过度繁复偏离原图简洁风")
    "busy, cluttered, overcrowded, over-decorated, excessive ornament, ornate filigree, "
    "scrollwork, fleur-de-lis, baroque, intricate detail, decorative swirls, lace pattern, "
    "too many elements, visual noise, decorative clutter"
)
...
GLOBAL_POS = (
    ...
    "a clean bottom banner below the emblem, "
    # 排版: 居中对称
    ...
    "the bottom banner is intentionally clean blank light purple, no decoration added by SDXL, "
...
REGIONS = [
    # 中心徽章: bat 旋转 + 徽章外环装饰
    {
        ...
        "prompt": (
            ...
            "the bat is rotated 30 degrees to its right and its wings tilted asymmetrically, "
            "the emblem has a simple thin black outline ring, no heavy decoration on the outside, no scrollwork, no fleur-de-lis, no ornate filigree, plain clean minimal, no compass, no dial, no clock face, no watch face, no pointer, no needle, no gauge, no sun dial, "
            "no triangle above the bat, no column eyes, no eye symbols, no extra geometric symbols, "
    ...
    # 顶部弧形带: v249 改成完全留白 (SDXL 不画任何东西, 装饰花纹 + 烧字全 PIL 后期)
    {
    ...
    # 底部饰带: v249 完全留白 (与顶弧一致, SDXL 不画任何东西, 烧字全 PIL 后期)
    {
        "x": 0.15, "y": 0.78, "w": 0.70, "h": 0.18, "strength": 0.40,
        "prompt": (
            "a clean smooth solid light purple #B57BC8 area at the bottom, "
            "NO decoration, NO pattern, NO fleur-de-lis, NO scrollwork, NO ornament, "
            "NO letter shapes, NO character forms, NO alphabet, NO readable text, "
            "NO specific characters, NO individual letters, NO capital letters, NO lowercase letters, "
            "NO letterform patterns, NO inscribed letters, NO embossed letters, NO carved letters, "
            "NO Roman letters, NO Greek letters, NO Cyrillic letters, NO faux text, "
            "the bottom band is intentionally blank, the actual decorative band and text will be added later, "
            "just a flat solid light purple color, no anything else. " + COHESIVE
        )
    },
]
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v249_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v249] submitted pid={pid[:8]} denoise={DENOISE} ipa={IPA_WEIGHT} (strict original style + vector + post-burn)")
    for _ in range(120):
        time.sleep(5)
        try:
            h = history(pid)
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v249_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v249] NOT FOUND raw_path")
        return None
    return raw_path
...
# ==================== v249 新增: PIL 后期烧非侵权词 (main 末尾, 不经过 SDXL 不会糊) ====================
...
def main():
    out_final = JOB / "v249_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        try:
            ...
            print("  [v249] top arc burn OK: LVMEN NOCTIS")
        except Exception as e:
            print(f"  [v249] top arc burn FAIL: {e}")
        try:
            ...
            print("  [v249] bottom word burn OK: NOCTWING + MORS VINI")
        except Exception as e:
            print(f"  [v249] bottom word burn FAIL: {e}")
        try:
            burn_est_year(final_img, int(final_img.height * 0.022))
            print("  [v249] est year burn OK: Est. 1862")
        except Exception as e:
            print(f"  [v249] est year burn FAIL: {e}")
        ...
        print(f"  saved v249_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v249.jpg"
