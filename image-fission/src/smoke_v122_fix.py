"""v122 修复版：纯色锁色 + 黑树锁黑 + 去边框 + 字体融合
用户反馈（2026-08-27）：
1. illust_1 纯黑白出现新颜色 → IPAdapter 0.55 + 黑白强锁 + denoise 0.60
2. camo_4 黑树变白 → prompt 明确 black palm tree silhouettes
3. 一律不要边框 → 负向禁 frame/border + 生成后四边裁 8px 去杂边
4. 字体按画风融合 → 新字体（MetalMania/PirataOne/Rye）+ 烧字脚本分离

用法：python src/smoke_v122_fix.py
"""
...
REFS = [
    {   # 纯色 → 0.55 + 黑白锁（修正跑色）
        "id": "illust_1", "ref_img": "pinterest_illust_1.jpg", "weight": 0.55, "denoise": 0.60,
        "subject": "elegant peacock silhouette with elaborate flowing tail feathers, baroque vector style",
        "palette": "pure black background, pure white linework only, strictly monochrome black and white, no other colors",
    },
    {   # 纯色 → 0.55（修正）
        "id": "denim_3", "ref_img": "pinterest_denim_3.jpg", "weight": 0.55, "denoise": 0.62,
    ...
    {   # 复杂 → 0.70
        "id": "eagle_2", "ref_img": "pinterest_eagle_2.jpg", "weight": 0.70, "denoise": 0.72,
    ...
    {   # 复杂 → 0.75 + 黑树锁黑（修正树变白）
        "id": "camo_4", "ref_img": "pinterest_camo_4.jpg", "weight": 0.75, "denoise": 0.72,
        "subject": "tropical palm tree with SOLID BLACK silhouette and dark black trunks and black palm leaves, classic military camouflage pattern background",
        "palette": "tropical jungle camouflage background, brown green tan khaki black palette, black palm tree silhouettes, organic camo blob shapes",
    },
    {   # 复杂 → 0.70
        "id": "skull_5", "ref_img": "pinterest_skull_5.jpg", "weight": 0.70, "denoise": 0.72,
    ...
    {   # 复杂 → 0.70
        "id": "metal_6", "ref_img": "pinterest_metal_6.jpg", "weight": 0.70, "denoise": 0.72,
...
POS_TAIL = (
    ...
    "fills entire frame edge-to-edge, full bleed, no margins, no halftone, no noise, no grain"
)
...
def build(ref, seed):
    ...
    g["9"] = {"class_type": "KSampler", "inputs": {
        ...
        "sampler_name": "dpmpp_2m", "scheduler": "karras", "denoise": ref["denoise"]}}
    ...
    g["14"] = {"class_type": "SaveImage",
               "inputs": {"images": ["13", 0],
                          "filename_prefix": f"v122_{ref['id']}_w{int(ref['weight']*100):03d}"}}
    return g
...
def run(ref, seed, out_base):
    ...
    r = requests.post(f"{COMFYUI}/prompt",
                      json={"prompt": build(ref, seed),
                            "client_id": f"v122_{int(time.time())}"}, timeout=10)
...
def main():
    out = PROJECT_ROOT / "jobs" / f"smoke_v122_{int(time.time())}"
    out.mkdir(parents=True, exist_ok=True)
    print("=== v122 修复版 6 张（锁色/黑树/去边框）===", flush=True)
    seed = 400301
    for ref in REFS:
