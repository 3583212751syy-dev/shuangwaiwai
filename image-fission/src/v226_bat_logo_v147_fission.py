"""
v226 — bat_logo 真裂变（v225 同源，更极简构图）

v225 落点 edge 1.31×（压在 1.3× 线上），最后一道缺口。
v226 仅继续收紧构图（同 seed 220001，受控对比）：
  - 去掉 v225 的 "thin ornamental crescent moon"（顶部弯月）
  - "baroque CURVES / curlicues" 退化为 "thin clean minimal circular ring outline"（细净环线，无繁复卷草）
  - 顶部 arched band 退化为单条极简细弧线，底部 element 保持 minimal
  - 保留：单只居中剪影蝙蝠 / denoise 0.80 / lora 0.50 / ipa 0.18 / canny 0.25 / tile 0.60 / NMKD 4x / 配色锁
目标：edge 压到 <1.3× 同时 laplacian ≥0.75×，达成干净 QC PASS。
"""
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v226_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v226] submitted pid={pid[:8]} denoise={DENOISE} lora={LORA_DETAIL} (v221 + cleaner composition, same seed, controlled)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v226_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v226] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v226_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v226_bat_logo_raw.jpg"
        ...
        print(f"  saved v226_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v226.jpg"
