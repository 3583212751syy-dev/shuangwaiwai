"""v319d: Tighten LaMa mask to ONLY pure-black text cores (gray<40),
NOT anti-aliased edges. Anti-alias edges + chain + dog tag all preserved
unchanged. Then LaMa produces minimal hallucination, no 'halo' / 'blur band'.

Key change vs v319c:
  - text mask = gray<40 (NOT gray<80). Catches only the dense letter cores.
  - NO dilate (0 instead of 2x2). Mask footprint minimal.
  - Drop is_serif entirely. ARMED horizontal crossbars stay as chain (preserved).
  - edge_smoothness=1 (was 3). Mask edge blur minimal.
  - Post-LaMa: copy original RGB back for chain_mask + dog_tag_mask regions
    (defensive copy-back even though LaMa shouldn't have touched them).
"""
import cv2
import numpy as np
import shutil
...
OUT_DIR = str(PROJECT / "jobs" / "v319d")
...
# Step 1: chain + dog tag detection (preserved untouched)
...
for band in BANDS_DEF:
    ...
    for i in range(1, n):
        ...
        # ALL CCs become "chain" — only the letter cores (gray<40) become text
        x0 = stats[i, cv2.CC_STAT_LEFT]
        y_loc = stats[i, cv2.CC_STAT_TOP]
        cc = (labels[y_loc:y_loc+h, x0:x0+w] == i)
        chain_mask[y0+y_loc:y0+y_loc+h, x0:x0+w][cc] = 255

# Dog tag detection: medium-gray region (80<gray<180) inside ARMED/FORCES bands
# with substantial area + rectangular aspect ratio
...
for band in BANDS_DEF:
    ...
    for i in range(1, n):
        ...
        h = stats[i, cv2.CC_STAT_HEIGHT]
        a = stats[i, cv2.CC_STAT_AREA]
        # dog tag is medium-sized, taller than wide
        if 8000 < a < 30000 and h > 80 and 0.5 < w/h < 1.5:
            x0 = stats[i, cv2.CC_STAT_LEFT]
            y_loc = stats[i, cv2.CC_STAT_TOP]
            cc = (labels[y_loc:y_loc+h, x0:x0+w] == i)
            dog_tag_mask[y0+y_loc:y0+y_loc+h, x0:x0+w][cc] = 255
            print(f'[v319d] {band["name"]}: detected dog-tag-like CC area={a} w={w} h={h}')

# Step 2: text mask = ONLY pure-black letter cores (gray<40)
# chain_mask + dog_tag_mask subtracted
text_mask_global = (gray < 40)
text_mask = text_mask_global & (chain_mask == 0) & (dog_tag_mask == 0)
print(f'[v319d] text mask px (gray<40 cores): {int(text_mask.sum())}')

# Visualize mask
mask_vis = np.zeros((H, W, 3), dtype=np.uint8)
mask_vis[text_mask] = [255, 0, 0]
mask_vis[chain_mask > 0] = [0, 255, 0]
mask_vis[dog_tag_mask > 0] = [255, 255, 0]  # yellow for dog tag
...
print('[v319d] Running LaMa inpaint (mask tight, no dilate, edge=1)...')
...
for b in BANDS_DEF:
    ...
    print(f'[v319d] {b["name"]}: capH={capH} fsize={fsize} "{text}" nat_w={nat_w} spacing={spacing} '
...
OUT = Path(OUT_DIR) / 'armed_text_only_v319d.jpg'
...
combined.save(str(Path(OUT_DIR) / 'armed_compare_v319d.jpg'), quality=90)
...
band_compare.save(str(Path(OUT_DIR) / 'armed_text_band_compare_v319d.jpg'), quality=95)
print(f'[v319d] Saved {OUT}')