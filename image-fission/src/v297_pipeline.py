"""
v297 — 清除中心圆盘内 BACARDÍ 字顶部残留（y>940 非蝙蝠区全填紫底）。
...
SUBJECT_VARIANTS = [
    ("up",     "NOCTAVEN", "DISTILLERY", "SHADOW OF THE WING",      297001),
    ("spread", "DUSKBAT",  "RESERVE",    "WINGS OF TWILIGHT",       297101),
    ("fold",   "MOONBAT",  "NOCTURNE",   "GUARDIAN OF THE DARK",    297201),
]
...
def make_clean_source():
    ...
    Image.fromarray(debug_img).save(r"E:/Desktop/双接口/image-fission/jobs/v297/_debug_mask.png")
    ...
    # 5) 关键：y > 940（蝙蝠尾巴以下）且非蝙蝠的一切像素全填紫底
    #    —— 去掉 BACARDÍ/MCKHEART 字顶部在中心圆盘内的残留
    ys_grid, xs_grid = np.mgrid[0:h, 0:w]
    lower_zone = ys_grid > 940
    not_bat = bat_mask_pre == 0
    lower_clear = lower_zone & not_bat
    clean[lower_clear] = bg_color
    print(f"[lower] y>940 非蝙蝠区清除 {int(lower_clear.sum())}px")
...
def main():
    ...
    src_path = COMFY_INPUT / "v297_text_free_source.png"
    ...
    if not src_path.exists() or args.force:
        ...
        print(f"[v297] clean source -> {src_path}")
    else:
        print(f"[v297] reuse clean source")
    ...
    for tag, big, sub, arc, _ in SUBJECT_VARIANTS:
        ...
        composed.save(JOB / f"v297_{tag}_composed.png", quality=95)
        ...
        final_path = JOB / f"v297_{tag}_final.png"
        final.save(final_path, quality=95)
        files.append(final_path)
        print(f"[v297] {tag} final -> {final_path.name}")
    ...
    gp = JOB / "_grid_v297.png"
    grid.save(gp, quality=92)
    print(f"[v297] grid -> {gp}")
