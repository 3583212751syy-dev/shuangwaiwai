"""
v225 — bat_logo 真裂变（v221 同源，更干净构图）

v221 为锐度/配色最优真裂变，但构图偏繁（two bats + 5 hanging beads 常读作底部吊坠残留）。
v225 仅改构图 prompt（同 seed 220001，受控）：
  - 去掉 "two bats" → 单只大胆居中剪影蝙蝠
  - 去掉 "5 ornamental hanging beads" / dangling charms → 底部干净无悬挂物
  - 保留：denoise 0.80 / lora 0.50 / ipa 0.18 / canny 0.25 / tile 0.60 / NMKD 4x / 配色锁
目标：构图更干净利落的真裂变，便于用户目检是否去除了底部吊坠观感。
"""
...
GLOBAL_POS = (
    ...
    "clean decorative baroque CURVES and curlicues woven into the badge as purely graphical motifs "
    "(abstract letterform-shaped flourishes, NOT forming any readable word, NOT spelling anything, NOT any brand name), "
    "the emblem ring is rotated 18 degrees and elegantly distorted with baroque flourishes, "
    "a thin ornamental crescent moon sits at the top of the ring, "
...
REGIONS = [
    # 中心徽章：单只大胆剪影蝙蝠 + 干净巴洛克框（去 two-bats / 去 hanging beads）
    {
        ...
        "prompt": (
            ...
            "clean decorative baroque CURVES and curlicues woven into the lower curve of the badge "
            ...
            "a thin ornamental crescent moon replaces the original moon at the top of the ring, "
            "the emblem ring is elegantly twisted with baroque flourishes, NO hanging beads, NO dangling charms, "
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v225_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v225] submitted pid={pid[:8]} denoise={DENOISE} lora={LORA_DETAIL} (v221 + cleaner composition, same seed, controlled)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v225_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v225] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v225_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v225_bat_logo_raw.jpg"
        ...
        print(f"  saved v225_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v225.jpg"
