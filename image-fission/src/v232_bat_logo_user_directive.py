"""
v232 — bat_logo 真裂变（用户硬指令版）

用户原话（2026-09-02 16:16）：
  "主体图片改变内容但是与原图有关联，风格跟结构保留原图的听得懂吗，颜色也不要改"

v232 严格按这四条执行：
  1) 结构保留    —— 三层布局：顶部弧形带 / 中间圆形徽章含 bat / 底部饰带，不能动
  2) 主体方向一致 —— bat 居中朝上展翅，与原图 bat 形态关联（可换装饰元素围绕，但不换物种）
  3) 风格保留    —— 2D flat printed vintage craft spirits label
  4) 颜色不改    —— 紫色调 1:1 锁死（Reinhard LAB + NEG 禁灰/禁漂色）

v229/v230/v231 全崩坏（3D 茶杯 / 字符乱拼 / bat 倒挂）已废弃。
"""
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v232_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v232] submitted pid={pid[:8]} denoise={DENOISE} canny={CANNY_STRENGTH} tile={TILE_STRENGTH} ipa={IPA_WEIGHT}")
    for _ in range(120):
        time.sleep(5)
        try:
            h = history(pid)
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v232_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
...
def main():
    out_final = JOB / "v232_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v232_bat_logo_raw.jpg"
