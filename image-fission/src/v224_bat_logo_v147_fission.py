"""
v224 — bat_logo 真裂变（v221 同源，换 upscaler 隔离 edge 源）

v222/v223 结论：lora/USM 调参、换 seed 都不能把 edge 压到 1.3× 内，且 v223 还漂色。
v224 假设：edge 膨胀来自 4x_NMKD-Siax upscaler 注入高频细节（输出 3840×5120）。
改 lanczos 软缩放（不注入细节）+ 去掉 USM，看 edge 是否落到 1.3× 内；
清晰度（laplacian）只要 ≥ 0.75× 即过关，软缩放预期更低但不会崩。
其余（denoise 0.80 / lora 0.50 / ipa 0.18 / canny 0.25 / tile 0.60 / seed 220001 / 配色锁）全同 v221。
受控：同 seed，仅换 upscaler。


v220 残留：
  - edge_density 1.45× （真裂变固有纹理，接受）
  - 1 个新色 (156,101,149) 淡紫灰 → 轻微违反配色铁律
  Reinhard LAB 只重映射 L 通道（明度），a/b 色相保留 dst 原样，
  所以 SDXL 若生成低饱和灰紫，迁移后仍是灰紫（原图没有）。
  → 根因在 SDXL 必须生成高饱和紫，源修靠 prompt 配色锁（守铁律 #11 不靠 PIL 盖）。

v221 改动（v220 基线不动，仅加配色约束）：
  - GLOBAL_POS 加 "strictly saturated purple/violet/black/white only, NO gray,
    NO desaturated muted purple, NO grayish tones, NO washed-out pale purple"
  - NEG 加 "desaturated purple, muted gray, grayish purple, dull tones,
    washed out, pale gray, off-palette gray, low-saturation"
  - 其余参数（denoise 0.80 / lora 0.50 / ipa 0.18 / canny 0.25 / tile 0.60 / seed 220001）全同 v220
  - 受控实验：同 seed + 仅改配色 prompt，隔离颜色变量
"""
...
GLOBAL_POS = (
    ...
    "MINIMAL bottom decoration, NO floating pendant below the badge, NO dangling charm, "
...
REGIONS = [
    # 中心徽章：保留徽章骨架 + 剪影蝙蝠重画 + 居中
    {
        ...
        "prompt": (
            ...
            "MINIMAL bottom decoration, NO pendant, NO teardrop. " + COHESIVE
        )
    },
    # 顶部弧形带：装饰性图案带
    {
...
def build(seed):
    ...
    # v224: 不用 4x_NMKD-Siax（注入高频→edge 膨胀），改 lanczos 软缩放到同尺寸，隔离 upscaler 是否为 edge 源
    g["14"] = {"class_type": "ImageScale", "inputs": {
        "image": ["12", 0], "width": 3840, "height": 5120, "upscale_method": "lanczos", "crop": "disabled"}}
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v224_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v224] submitted pid={pid[:8]} denoise={DENOISE} lora={LORA_DETAIL} (v221 + lanczos upscale, no NMKD, controlled)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v224_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v224] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v224_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        # v224: lanczos 已是软缩放，跳过 USM（否则重新拉高 edge）
        out_raw = JOB / "v224_bat_logo_raw.jpg"
        ...
        print(f"  saved v224_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v224.jpg"
