"""
v284 — 用本地 BrushNet (GitHub 模型) 做高质量 inpaint，解决 v283 三类问题：
  1. 文字背景遮挡 → 局部采样背景色填充，不做全图大模糊
  2. 主体缺失/像素缺失 → BrushNet + 外扩生成 mask 给翅膀留空间，严格合成 mask 贴回
  3. 背景颜色改变 → 合成 mask 只覆盖蝙蝠区，背景 100% 不动

核心变化：
  - 弃用 VAEEncodeForInpaint，改用 ComfyUI-BrushNet (diffusion_pytorch_model.safetensors SDXL)
  - 生成 mask 比蝙蝠剪影大 60–80px，让 SDXL 能画出完整翅膀
  - 合成 mask 仍是蝙蝠剪影 + 15px 缓冲，用 BlendInpaint 消除硬边
  - 文字区：局部中值背景色 + 15px 羽化，不再全图统一模糊
"""
import argparse
import json
import math
...
SUBJECT_VARIANTS = [
    ("up",     "NOCTAVEN", "DISTILLERY", "SHADOW OF THE WING",  284001),
    ("spread", "DUSKBAT",  "RESERVE",    "WINGS OF TWILIGHT",   284101),
    ("fold",   "MOONBAT",  "NOCTURNE",   "GUARDIAN OF THE DARK", 284201),
]
...
NEG = (
    ...
    "missing pixels, holes, transparent, low quality, cropped wings"
)
...
def _local_median_fill(arr, text_mask, k=15):
    """对 text_mask 内每个像素，用周围 k×k 非 text 像素的中值填充。"""
    h, w = arr.shape[:2]
    text_u8 = (text_mask > 127).astype(np.uint8) * 255
    inv = 255 - text_u8
    # 快速近似：对每个偏移采样非 text 像素并累加，最后取中值近似用平均
...
def make_text_free_source():
    """精确去字：弧带用局部中值填充保留渐变；主/副/Est/1862 用均匀背景填充；
    圆环/蝙蝠/内徽章 100% 不动，不做全图大模糊。"""
    orig = Image.open(COMFY_INPUT / ORIG).convert("RGB")
    arr = np.array(orig).astype(np.float32)
    h, w = arr.shape[:2]

    # 1. 弧带文字：局部中值填充，保留横幅渐变
...
def make_generation_mask(size=(1552, 2000)):
    """BrushNet 生成 mask：比蝙蝠剪影外扩 70px，给翅膀留足空间。"""
    w, h = size
    bat = np.array(make_bat_mask().convert("L")).astype(np.uint8)
    _, bat = cv2.threshold(bat, 80, 255, cv2.THRESH_BINARY)
    mask = cv2.dilate(bat, np.ones((70, 70), np.uint8), iterations=1)
    # 限制在 inner badge + 一点缓冲内，防止碰圆环
    badge = np.zeros((h, w), np.uint8)
    cv2.circle(badge, (cx, cy), r_badge + 35, 255, -1)
    mask = cv2.bitwise_and(mask, badge)
    mask = cv2.GaussianBlur(mask, (25, 25), 0)
    return Image.fromarray(mask)


def make_composition_mask(size=(1552, 2000)):
    """最终合成 mask：蝙蝠剪影 + 15px 缓冲，边缘羽化。"""
    w, h = size
    bat = np.array(make_bat_mask().convert("L")).astype(np.uint8)
    _, bat = cv2.threshold(bat, 80, 255, cv2.THRESH_BINARY)
    mask = cv2.dilate(bat, np.ones((15, 15), np.uint8), iterations=1)
...
def make_pose_guides():
    """生成三姿态 Canny 引导边线图。基于生成 mask 范围，让翅膀更夸张。"""
    bat = np.array(make_bat_mask().convert("L")).astype(np.uint8)
    ...
    poses = {}
    up = _transform_silhouette(bat, sx=0.60, sy=1.55, rot_deg=-3)
    spread = _transform_silhouette(bat, sx=1.80, sy=0.55, rot_deg=0)
    fold_base = _transform_silhouette(bat, sx=1.10, sy=0.85, rot_deg=2)
    h, w = fold_base.shape
    ys = np.arange(h).reshape(-1, 1).astype(np.float32)
    factor = 1.0 + 0.35 * np.clip((ys - cy) / (h - cy), 0, 1) - 0.10 * np.clip((cy - ys) / cy, 0, 1)
    ...
    for tag, m in (("up", up), ("spread", spread), ("fold", fold)):
        ...
        path = COMFY_INPUT / f"v284_pose_{tag}.png"
        poses[tag].save(path, quality=95)
        print(f"[v284] pose guide {tag} -> {path}")
    return poses
...
def build(seed, tag, pose_img):
    bat_desc = {
        "up": (
            ...
            "elongated silhouette, wings pointing to top center, full intact wings"
        ),
        "spread": (
            "bat wings spread wide horizontally, powerful broad wingspan, "
            "wings flat and stretched left and right, open stance, full intact wings"
        ),
        "fold": (
            "bat wings folded downward draping like dark cloak, "
            "wings wrapped around lower body, compact hunched posture, full intact wings"
        ),
    }[tag]
    ...
    pos = (
        ...
        f"solid black bat, no holes, no missing pixels, crisp silhouette"
    )

    g = {}
    g["1"] = {"class_type": "CheckpointLoaderSimple", "inputs": {"ckpt_name": CKPT}}
    g["2"] = {"class_type": "LoadImage", "inputs": {"image": "v284_text_free_source.png"}}
    g["3"] = {"class_type": "ImageScaleToTotalPixels", "inputs": {
        "image": ["2", 0], "upscale_method": "lanczos", "megapixels": 1.2, "resolution_steps": 64}}

    # BrushNet 高质量 inpaint
    g["brush_load"] = {"class_type": "BrushNetLoader", "inputs": {"brushnet": BRUSHNET, "dtype": "float16"}}
    g["mask_load"] = {"class_type": "LoadImage", "inputs": {"image": "v284_generation_mask.png"}}
    ...
    # 先基于基础 clip 编码 prompt，避免和 BrushNet 形成循环
    g["pg0"] = {"class_type": "CLIPTextEncode", "inputs": {"clip": ["1", 1], "text": pos}}
    g["ng0"] = {"class_type": "CLIPTextEncode", "inputs": {"clip": ["1", 1], "text": NEG}}

    g["brush"] = {"class_type": "BrushNet", "inputs": {
        "model": ["1", 0], "vae": ["1", 2], "image": ["3", 0], "mask": ["mask_conv", 0],
        "brushnet": ["brush_load", 0], "positive": ["pg0", 0], "negative": ["ng0", 0],
        "scale": BRUSHNET_SCALE, "start_at": 0, "end_at": 10000}}

    g["5"] = {"class_type": "IPAdapterUnifiedLoader", "inputs": {"model": ["brush", 0], "preset": "PLUS (high strength)"}}
    ...
    g["10"] = {"class_type": "KSampler", "inputs": {
        "model": ["7", 0], "positive": ["24", 0], "negative": ["brush", 2],
        "latent_image": ["brush", 3], "seed": seed, "steps": 32, "cfg": 6.5,
        "sampler_name": "dpmpp_2m_sde_gpu", "scheduler": "karras", "denoise": DENOISE}}
    g["11"] = {"class_type": "KSampler", "inputs": {
        "model": ["7", 0], "positive": ["24", 0], "negative": ["brush", 2],
        "latent_image": ["10", 0], "seed": seed + 1, "steps": 20, "cfg": 6.5,
        "sampler_name": "dpmpp_2m_sde_gpu", "scheduler": "karras", "denoise": REFINE_DENOISE}}
    g["12"] = {"class_type": "VAEDecode", "inputs": {"samples": ["11", 0], "vae": ["1", 2]}}
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["12", 0], "filename_prefix": f"v284_{tag}"}}
    return g
...
def main():
    ...
    src_path = COMFY_INPUT / "v284_text_free_source.png"
    gen_mask_path = COMFY_INPUT / "v284_generation_mask.png"
    comp_mask_path = COMFY_INPUT / "v284_composition_mask.png"
    bat_mask_path = COMFY_INPUT / "v284_bat_mask.png"
    ...
    if not src_path.exists() or args.force:
        ...
        print(f"[v284] text-free source -> {src_path}")
        print(f"[v284] source check -> {JOB / '_source_check.png'}")
    else:
        print(f"[v284] reuse text-free source")

    if args.source_only:
        print("[v284] source-only mode, exit.")
        return

    if not bat_mask_path.exists() or args.force:
        bm = make_bat_mask()
        bm.save(bat_mask_path)
        print(f"[v284] bat mask -> {bat_mask_path}")
    else:
        print(f"[v284] reuse bat mask")

    pose_guides = make_pose_guides()

    if not gen_mask_path.exists() or args.force:
        gm = make_generation_mask()
        rgba = Image.merge("RGBA", (gm, gm, gm, gm))
        rgba.save(gen_mask_path)
        print(f"[v284] generation mask -> {gen_mask_path}")
    else:
        print(f"[v284] reuse generation mask")

    if not comp_mask_path.exists() or args.force:
        cm = make_composition_mask()
        rgba = Image.merge("RGBA", (cm, cm, cm, cm))
        rgba.save(comp_mask_path)
        print(f"[v284] composition mask -> {comp_mask_path}")
    else:
        print(f"[v284] reuse composition mask")

    text_free = Image.open(src_path).convert("RGB")
    comp_mask = Image.open(comp_mask_path).convert("L")
    comp_mask_arr = np.array(comp_mask).astype(np.float32) / 255.0
    comp_mask_arr = np.stack([comp_mask_arr] * 3, axis=-1)

    files = []
    for tag, big, sub, arc, seed in SUBJECT_VARIANTS:
        cands = sorted(COMFY_OUTPUT.glob(f"v284_{tag}_*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if not cands or args.force:
            print(f"[v284] generating {tag} ...")
            ...
            raw_path = cands[0]
            print(f"[v284] reuse {tag}: {raw_path.name}")
        ...
        composed = raw_arr * comp_mask_arr + bg_arr * (1 - comp_mask_arr)
        composed = Image.fromarray(np.clip(composed, 0, 255).astype(np.uint8))
        composed.save(JOB / f"v284_{tag}_composed.png", quality=95)
        ...
        final_path = JOB / f"v284_{tag}_final.png"
