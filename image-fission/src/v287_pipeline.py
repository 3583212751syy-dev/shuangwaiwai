"""
v287 — v286 基础上改进：
  1. 弧带文字用更大窗口局部中值 + 二次检测，彻底消除 ghost
  2. 蝙蝠提取前先填充为纯黑剪影，再形变，避免边缘发虚/背景混入
  3. 三种姿态在 v286 参数上微调，让形变更自然
"""
import argparse
import math
import sys
...
SUBJECT_VARIANTS = [
    ("up",     "NOCTAVEN", "DISTILLERY", "SHADOW OF THE WING",  287001),
    ("spread", "DUSKBAT",  "RESERVE",    "WINGS OF TWILIGHT",   287101),
    ("fold",   "MOONBAT",  "NOCTURNE",   "GUARDIAN OF THE DARK", 287201),
]
...
def _fill_text_region_local(arr, region_mask, feather=19, threshold=8, passes=2):
    """多次局部中值填充，彻底消除文字 ghost。"""
    h, w = arr.shape[:2]
    for _ in range(passes):
        ...
        bg = _local_median_fill(arr, text_mask, k=35, step=2)
...
def make_text_free_source():
    ...
    arr = _fill_text_region_local(arr, arc_region, feather=23, threshold=6, passes=2)
...
def make_variants():
    ...
    variants = {}
    variants["up"] = _warp_bat(bat_pixels, mask, sx=0.80, sy=1.22, rot_deg=-2)
    variants["spread"] = _warp_bat(bat_pixels, mask, sx=1.38, sy=0.72, rot_deg=0)
    variants["fold"] = _warp_bat(bat_pixels, mask, sx=1.10, sy=0.90, rot_deg=1, fold_factor=0.22)
    return variants
...
def main():
    ...
    src_path = COMFY_INPUT / "v287_text_free_source.png"
    ...
    if not src_path.exists() or args.force:
        ...
        print(f"[v287] text-free source -> {src_path}")
    else:
        print(f"[v287] reuse text-free source")
    ...
    for tag, big, sub, arc, _ in SUBJECT_VARIANTS:
        ...
        composed.save(JOB / f"v287_{tag}_composed.png", quality=95)
        ...
        final_path = JOB / f"v287_{tag}_final.png"
        final.save(final_path, quality=95)
        files.append(final_path)
        print(f"[v287] {tag} final -> {final_path.name}")
    ...
    gp = JOB / "_grid_v287.png"
    grid.save(gp, quality=92)
    print(f"[v287] grid -> {gp}")
