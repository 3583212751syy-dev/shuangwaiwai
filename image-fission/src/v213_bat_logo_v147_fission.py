"""
v213 — bat_logo 真裂变（v147 锁基线 + v193 prompt 框架）

目标：BACARDÍ bat_logo (test_6978fabda2cc99629fa9e81f802762d3.jpg)
策略：
  1. v147 锁死基线参数 (denoise 0.80 / ipa 0.18 / lora 1.0 / canny 0.25 / tile 0.60)
  2. 字作为装饰图形元素一起重画，绝不 PIL 烧字
  3. 3 region: top arc / main badge / bottom banner
  4. 后置 Reinhard LAB 配色迁移（紫底锁定）
  5. OCR 自检：不能有任何可读字
  6. 出图左右对照（要求差异显著：徽章转角、蝙蝠重画、小元素改、骨架改）

历史教训（v196-v212 都是烧字伪裂变，已被用户打脸）：
  - 这次走 SDXL 整体重画，让徽章骨架真正变形
  - 不再擦字、不再换字、不再"1:1 排版复制"
"""
...
SEED = 213001  # 与 v193 (700510)/v188 (700508)/v185 (700501)/v164 (700401) 不同
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v213_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v213] submitted pid={pid[:8]} denoise={DENOISE} ipa={IPA_WEIGHT} (v147 base)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v213_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v213] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v213_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v213_bat_logo_raw.jpg"
        ...
        print(f"  saved v213_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
