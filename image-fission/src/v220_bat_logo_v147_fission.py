"""
v220 — bat_logo 真裂变（源头消除底部装饰，不靠 PIL 修补）

v216-v219 教训：
  - 低 denoise 会触发 SDXL reading 模式（原图字位置被填 BACLRET/SMIRAICE 假词，违反铁律 #1）
  - PIL 修底部装饰治标不治本

v220 设计（守铁律 #1 + 降 edge + 源头去装饰）：
  - DENOISE = 0.80（高，避免 SDXL reading 模式）
  - LORA_DETAIL = 0.50（低，降纹理密度 → edge 到 1.3× 内）
  - POS：保留 v215 的 centered + 2D flat silhouette，加 "minimal bottom decoration,
    NO floating pendant below the badge, NO dangling charm, clean empty space below"
  - NEG：保留 v215 全禁词（starburst / readable text / jewelry / pendant / teardrop / off-center）
  - 后处理：轻 USM 锐化主轮廓
技术栈：ComfyUI + ProteusV0.4 + IPAdapter Plus + Canny 0.25 + Tile 0.60
        + add-detail LoRA(0.50) + Reinhard LAB 配色迁移
"""
...
SEED = 220001  # 与 v213/214/215/216 都不同
...
# ==================== Prompt 框架（v220：源头去底部装饰 + 居中 + 矢量剪影）====================
GLOBAL_POS = (
...
REGIONS = [
    # 中心徽章：保留徽章骨架 + 剪影蝙蝠重画 + 居中
    {
        ...
        "prompt": (
            ...
            "perfectly centered and symmetrical, MINIMAL bottom decoration, NO pendant, NO teardrop. " + COHESIVE
        )
    },
    # 顶部弧形带：装饰性图案带
    {
    ...
    # 底部徽章饰带：装饰性图案（minimal）
    {
        ...
        "prompt": (
            ...
            "NOT pendant, NOT teardrop, NO hanging charm), kept purple and black palette, "
            "flat 2D vintage style, perfectly centered, MINIMAL. " + COHESIVE
        )
    },
]
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v220_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v220] submitted pid={pid[:8]} denoise={DENOISE} lora={LORA_DETAIL} (v215 skeleton + minimal bottom + high denoise to avoid SDXL reading mode)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v220_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v220] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v220_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v220_bat_logo_raw.jpg"
        ...
        print(f"  saved v220_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v220.jpg"
