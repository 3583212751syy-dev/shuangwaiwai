"""
v222 — bat_logo 真裂变（v221 同源，降 edge_density 向 1.3× 门槛收敛）

v221 残留唯一硬门槛 miss：edge_density 1.55× （> 1.3×）
根因两个直接杠杆：
  1. LORA_DETAIL=0.50 注入过多细节 → 自然边缘增多
  2. 后置 USM (radius=1.5, percent=50) 主动加锐 → 边缘像素膨胀
v222 改动（v221 全参数 + 配色锁保留）：
  - LORA_DETAIL 0.50 → 0.35（少注入细节）
  - 后置：先 0.5px Gaussian 轻平滑，再 USM 降到 radius=1.0 percent=20 threshold=4
  - denoise 0.80 / ipa 0.18 / canny 0.25 / tile 0.60 / seed 220001 全同（受控对比）
目标：edge 压到 ~1.3× 内，整体/字体清晰度仍 ≥ 原图
"""
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v222_bat_logo"}}
    return g
...
def gen(seed):
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v222_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
...
def main():
    out_final = JOB / "v222_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        # v222: 先 0.5px Gaussian 轻平滑压 edge，再轻 USM（radius=1.0 percent=20）
        matched_rgb = matched_rgb.filter(ImageFilter.GaussianBlur(radius=0.5))
        matched_rgb = unsharp(matched_rgb, radius=1.0, percent=20, threshold=4)
        out_raw = JOB / "v222_bat_logo_raw.jpg"
        ...
        print(f"  saved v222_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v222.jpg"
