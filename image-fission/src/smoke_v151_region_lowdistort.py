"""v151 逐元素区域控制裂变 + 强抗失真（本机 ComfyUI / SDXL）。

方向：用户要求「回到 v147 那种逐元素大胆裂变，但减少失真」。
v147 的失真根因 = 区域提示只管「这块画什么」，但元素身份/真实感没被锚定，
模型从文字生成「通用鹰/通用骷髅」→ 假、糊、穿模。

v151 抗失真三招（均经 v149 验证有效）：
1. 强 IP-Adapter 参考原图 weight 0.18→0.50：锁住元素真实身份（这是治失真最强杠杆）
2. Tile CN 0.60→0.75：边缘/细节更连贯，不融化、不穿模
3. 分辨率 1.2MP→1.5MP：每元素像素预算↑，不糊
保留 v147 的 bold 区域裂变（鹰正对/骷髅3-4朝向深裂/火焰斜升/铁链3段带尖刺/顶部皇冠），
并给每条区域 prompt 加 photorealistic / real texture 真实感短语。

IP-Adapter 用 style transfer + 高权重：锁「元素身份/配色画风」，但因 denoise 0.78 仍高，
区域 prompt 仍可驱动角度/大小/数量变化（不锁构图）。

用法：python smoke_v151_region_lowdistort.py [ref_id]   (默认 eagle_2)
输出：jobs/smoke_v151/v151_{id}.jpg
"""
...
DENOISE = 0.78                      # 略收，配合强 IPA 保身份（0.78 仍足够让区域驱动变化）
IPA_WEIGHT = 0.50                   # v147:0.18 → 0.50 强锁元素身份（治失真主杠杆）
LORA_DETAIL = 1.0                   # 抗失真
MEGA_PIXELS = 1.5                   # v147:1.2 → 1.5 高分辨率给每元素更多像素

# 空间一致性 ControlNet（v151 增强）
CN_CANNY = "controlnet-canny-sdxl-1.0.fp16.safetensors"
CN_TILE = "controlnet-tile-sdxl-1.0.safetensors"
CANNY_STRENGTH = 0.22   # v147:0.25 → 0.22 仍锁位置，但放一点角度空间
TILE_STRENGTH = 0.75    # v147:0.60 → 0.75 强保细节/边缘连贯，不融化
REGION_STRENGTH_SCALE = 0.65  # v147:0.55 → 0.65 让角度/数量变化仍生效

# 全局负向：锁配色 + 无字 + 抗失真 + 抗穿模 + 抗元素融合 + 抗假AI感
NEG_BASE = (
...
REFS = [
    {
        ...
        "regions": [
            # 老鹰：正对镜头、双翼对称展开 —— 改朝向/姿态；强真实感
            {"x": 0.10, "y": 0.00, "w": 0.80, "h": 0.42, "strength": 1.25,
             "prompt": ("a photorealistic bald eagle FACING THE CAMERA head-on (NOT a side profile), "
                        ...
                        "fierce open beak, real white and silver feathers with fine detailed texture, "
                        "centered dominant focal element at the top with a CLEAR BLACK GAP between it and any lower elements, "
                        "gothic style, black background. "
                        + COHESIVE)},
            # 骷髅：3/4 朝向、深裂痕、单颗（不戴皇冠，避免和顶部皇冠撞车）；强真实感
            {"x": 0.30, "y": 0.50, "w": 0.40, "h": 0.46, "strength": 1.35,
             "prompt": ("ONE single large photorealistic human skull turned at a three-quarter angle (NOT a front view), "
                        "deep realistic cracks and fractures across the bone surface, "
                        "no crown, no hat, just bone with cracked skullcap, "
                        "gray bone with realistic shadow and fine bone texture, "
                        "CLEAR BLACK GAP between this skull and any element above it, "
                        "centered lower area, gothic. "
                        + COHESIVE)},
            # 火焰：左下向右上斜升的火柱（改燃烧走势/方向）；真实火
            {"x": 0.00, "y": 0.22, "w": 0.22, "h": 0.74, "strength": 1.15,
             "prompt": ("realistic red and orange flames rising as a DIAGONAL sweeping column flowing from lower-left toward upper-right, "
                        "dynamic motion, asymmetric, no wrapping symmetry, "
                        "fires DO NOT touch the skull, separate with black space. "
                        + COHESIVE)},
            # 铁链：清晰 3 段铁链沿画面右下边→向上→顶部回折，带金属尖刺；强真实金属感
            {"x": 0.62, "y": 0.42, "w": 0.38, "h": 0.55, "strength": 1.30,
             "prompt": ("THREE DISTINCT segments of heavy photorealistic iron chain with sharp metal SPIKES, "
                        ...
                        "industrial gothic, gray metal with realistic metallic surface and highlights, bold SOLID LINK outlines, "
                        "each chain segment SEPARATED from skull and flame by clear black space, "
                        "CHAIN DOES NOT TOUCH the skull or eagle at all. "
                        + COHESIVE)},
            # 顶部小元素：原图后面小鹰换成哥特铁王冠；强真实金属感
            {"x": 0.36, "y": 0.00, "w": 0.28, "h": 0.12, "strength": 1.10,
             "prompt": ("a SINGLE clean photorealistic gothic iron crown with 5 visible spikes at the very top center, "
                        "realistic metallic surface, no birds anywhere near this area, "
                        "the crown SITS ALONE in its own black region, "
...
def build(ref, seed):
    ...
    # IPAdapter style 锁配色+元素身份（v151 提到 0.50）
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": f"v151_{ref['id']}"}}
    return g


def gen(ref, seed, out_base):
    tag = ref["id"]
    out = out_base / f"v151_{tag}.jpg"
    if out.exists() and out.stat().st_size > 100000:
        print(f"  [{tag}] 已存在，跳过", flush=True); return True
    import json as _json
    g = build(ref, seed)
    r = requests.post(f"{COMFYUI}/prompt", json={"prompt": g, "client_id": f"v151_{int(time.time())}"}, timeout=15)
    try:
        j = r.json()
    except Exception:
        j = {}
...
def main():
    ...
    out = PROJECT_ROOT / "jobs" / "smoke_v151"
