# -*- coding: utf-8 -*-
"""
smoke_v134_keepstructure.py -- 锁结构构图配色, 只裂变内部内容 (v134)

用户指令 (2026-08-28 收敛版):
  保留原图结构/构图/颜色不变, 其余内容裂变。
v133 病灶: subject 全是 "completely different / replace with"(换主体) + denoise 0.72
  太重 -> 结构被重画 (SSIM 0.02~0.23, 与原图几乎无关)。
v134 修正:
  - subject 全部改为 "保留主体轮廓/构图/位置, 重绘内部细节"
  - Canny 0.55 -> 0.62 (强锁结构骨架)
  - denoise 0.72 -> 0.62 (给结构更多保留空间)
  - 保留 Reinhard 保色 + 等原图尺寸 + PIL 叠字兜底
...
用法:
  python src/smoke_v134_keepstructure.py --test           # 只跑 denim_3
  python src/smoke_v134_keepstructure.py                  # 全量 6 张
  python src/smoke_v134_keepstructure.py --only skull_5   # 单图
"""
...
CONFIG = {
    "pinterest_denim_3.jpg": {
        "subject": ("denim patchwork butterfly collage, KEEP the same butterfly silhouette, "
                    "same position and same composition area, same background, but completely "
                    "redesigned wing patterns and internal patch details, same indigo denim "
                    "texture and stitch style"),
    ...
    "pinterest_camo_4.jpg": {
        "subject": ("camouflage pattern, KEEP the same palm jungle composition, same layout "
                    "and same foliage silhouette, but completely redesigned leaf pattern and "
                    "camouflage blotches inside, same olive green brown color family"),
        "palette": ("same exact colors as original: olive green, khaki brown, dark forest"),
        "regions": [],
    },
    "pinterest_eagle_2.jpg": {
        "subject": ("a majestic double-headed eagle crest with spread wings perched on a "
                    "weathered shield, KEEP the same eagle silhouette, same position, same "
                    "wingspan and same composition, but completely redesigned feather "
                    "details, flame patterns and shield ornament inside, high contrast "
                    "tattoo style"),
    ...
    "pinterest_illust_1.jpg": {
        "subject": ("black and white ornamental pattern with stylized blooms and vines, "
                    "KEEP the same symmetrical frame composition, same layout and same "
                    "border silhouette, but completely redesigned botanical species and "
                    "scrollwork inside the frame, intricate engraving style, ornate linework"),
        "palette": ("same exact colors as original: black ink on cream white paper"),
        "regions": [],
    },
    "pinterest_metal_6.jpg": {
        "subject": ("bronze carved eagle emblem on a shield with cracked stone, KEEP the "
                    "same eagle silhouette, same shield shape and same composition, but "
                    "completely redesigned feather and engraving details inside, weathered "
                    "metal engraving aesthetic"),
    ...
    "pinterest_skull_5.jpg": {
        "subject": ("ornate skull with thorny vines and roses, KEEP the same skull "
                    "silhouette, same position, same composition, but completely redesigned "
                    "vine pattern, rose arrangement and ornament details inside, gothic "
                    "dark romanticism"),
...
def build_restyle(base_name, subject, palette, seed, prefix):
    """Stage A: 内容裂变 -- Canny 0.62 锁构图骨架 + IPA 0.45 锁色系 + denoise 0.62 (v134: 保留结构)"""
    g = {}
    g["1"] = {"class_type": "CheckpointLoaderSimple", "inputs": {"ckpt_name": CKPT}}
    ...
    g["6"] = {"class_type": "ControlNetApplyAdvanced", "inputs": {
        ...
        "strength": 0.62, "start_percent": 0.0, "end_percent": 0.9, "control_net": ["5", 0]}}
...
def run_one(fname, cfg, ci, out_dir, ts, seed0):
    ...
    # Stage A: 内容裂变
    prefix = f"v134_{ts}_{tag}_fission"
    g = build_restyle(cur_name, cfg["subject"], cfg["palette"], seed0 + 1, prefix)
    pid = submit(g, f"v134_{ts}_{tag}_sA")
    if not pid:
    ...
    cur_pil = fission_1mp
    cur_name = f"v134_{ts}_{tag}_fission.png"
    ...
    if regions:
        for ri, r in enumerate(regions):
            ...
            ref_name = f"v134_{ts}_{tag}_r{ri}_ref.png"
            ref.save(INPUT / ref_name)
            mask = make_mask(cur_pil.size, [bx], dilate=14)
            mask_name = f"v134_{ts}_{tag}_r{ri}_mask.png"
            mask.save(INPUT / mask_name)
            redraw_prefix = f"v134_{ts}_{tag}_r{ri}"
            ...
            pid2 = submit(g2, f"v134_{ts}_{tag}_b{ri}")
            ai_ok = False
            if pid2:
                ...
                if o2:
                    ...
                    if up2:
                        ...
                        cur_name = f"v134_{ts}_{tag}_r{ri}_base.png"
            ...
            cur_name = f"v134_{ts}_{tag}_r{ri}_final.png"
            cur_pil.save(INPUT / cur_name)
    ...
    final_path = out_dir / f"v134_{tag}.png"
...
def main():
    ...
    args = ap.parse_args()
    ts = int(time.time())
    out_dir = ROOT / "jobs" / f"smoke_v134_keepstructure_{ts}"
    out_dir.mkdir(parents=True, exist_ok=True)
    print(f"=== smoke_v134_keepstructure -> {out_dir} ===", flush=True)
