...
REFS = [
    ...
    {"id": "denim_3",  "filename": "pinterest_denim_3.jpg",  "weight": 0.55, "cn_weight": 0.55,
...
def build_graph(ref):
    ...
    g["17"] = {"class_type": "SaveImage", "inputs": {"images": ["16", 0], "filename_prefix": "v135_clean"}}
    return g
...
def run_one(ref, out_dir):
    import requests
    tag = ref["id"]
    out_path = out_dir / f"v135_clean_{tag}.png"
    if out_path.exists() and out_path.stat().st_size > 100_000:
        print(f"[skip] {tag}: 已存在 {out_path.stat().st_size//1024}KB", flush=True); return True
    print(f"[run] {tag}  cn={ref['cn_weight']}  ipa={ref['weight']}  denoise=0.62", flush=True)
    g = build_graph(ref)
    try:
...
def main():
    out = PROJECT_ROOT / "jobs" / f"smoke_v135_{int(time.time())}"
    out.mkdir(parents=True, exist_ok=True)
    print(f"=== v135 (Canny cn=0.55 + IPA style=0.55 + denoise=0.62, 无Stage0) -> {out} ===", flush=True)
    ok = 0
    for ref in REFS:
        if run_one(ref, out):
            ok += 1
