"""v129: 强裂变档 — denoise 0.65 + Canny 0.35 + IPAdapter style 0.45.

目的: v128 (denoise0.55/Canny0.55/IPA0.55) SSIM≈0.85 太贴原图, 裂变感不足。
本版降低锁死权重、提高重绘强度, 让内容明显裂变, 但保留配色/构图骨架(同源)。
- 一阶 KSampler denoise 0.65 (内容重绘空间)
- Canny strength 0.35 (弱锁主结构, 防主体丢失)
- IPAdapter style weight 0.45 (锁画风/配色, 不锁内容)
- 二阶 KSampler denoise 0.18 (细化降噪)
先只出干净裂变图(不烧字), 供用户看裂变效果; 文字后续按原图位置另烧。
"""
...
REFS = [
    {"id": "illust_1", "filename": "pinterest_illust_1.jpg", "weight": 0.45, "cn_weight": 0.35,
    ...
    {"id": "eagle_2",  "filename": "pinterest_eagle_2.jpg",  "weight": 0.45, "cn_weight": 0.35,
    ...
    {"id": "denim_3",  "filename": "pinterest_denim_3.jpg",  "weight": 0.45, "cn_weight": 0.35,
    ...
    {"id": "camo_4",   "filename": "pinterest_camo_4.jpg",   "weight": 0.45, "cn_weight": 0.35,
    ...
    {"id": "skull_5",  "filename": "pinterest_skull_5.jpg",  "weight": 0.45, "cn_weight": 0.35,
    ...
    {"id": "metal_6",  "filename": "pinterest_metal_6.jpg",  "weight": 0.45, "cn_weight": 0.35,
...
def build_graph(ref):
    ...
    g["17"] = {"class_type": "SaveImage", "inputs": {"images": ["16", 0], "filename_prefix": "v129_strong"}}
    return g
...
def run_one(ref, out_dir):
    import requests
    tag = ref["id"]
    out_path = out_dir / f"v129_strong_{tag}.png"
    if out_path.exists() and out_path.stat().st_size > 100_000:
        print(f"[skip] {tag}: 已存在 {out_path.stat().st_size//1024}KB", flush=True); return True
    print(f"[run] {tag}  cn={ref['cn_weight']}  ipa={ref['weight']}  denoise=0.65", flush=True)
    g = build_graph(ref)
    try:
        r = requests.post(f"{COMFYUI}/prompt", json={"prompt": g, "client_id": f"v129_{int(time.time())}"}, timeout=15)
...
def main():
    ...
    print(f"=== v129 strong (Canny cn=0.35 + IPA style=0.45 + denoise=0.65) -> {out} ===", flush=True)
    ok = 0
    for ref in REFS:
        if run_one(ref, out):
            ok += 1
