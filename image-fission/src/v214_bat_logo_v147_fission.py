"""
v214 — bat_logo 真裂变（基于 v213，修两处违和）

v213 用户目检发现 2 处违和：
  1. 上方"炸裂星形"装饰无意义（SDXL 自加，非原图元素）
  2. 中央字母"TMG MME / CENT"含义不明（违反"单词须与图元素隐喻一致"）

v214 修复方向：
  - NEG 加 starburst / exploding star / firework / sunburst 等，禁止炸裂星形
  - 中央徽章文字改为"纯装饰 letterform 花纹，不拼成可读单词"
    （符合铁律 #1：禁止 PIL 烧可读字；同时也消除"含义不明"问题——
     因为没有可读单词，就不存在"含义对不对"）
  - 其余真裂变要素保留：SDXL 双蝙蝠重画 + 徽章转角 + 巴洛克卷草 + 锁色
技术栈：ComfyUI + ProteusV0.4 + IPAdapter Plus high + Canny 0.25 + Tile 0.60
        + add-detail LoRA + Reinhard LAB 配色迁移（同 v213 基线/v147 风格）
"""
...
NEG_BASE = (
    ...
    "calligraphy, scripture that spells a word, "
...
# ==================== Prompt 框架（v147 风格，强化差异化 + 无炸裂星形 + 无可读字）====================
GLOBAL_POS = (
    ...
    "NO starburst, NO exploding star, NO firework, NO spiky sunburst decoration"
)
...
REGIONS = [
    ...
    # 底部徽章饰带：装饰性图案
    {
        ...
        "prompt": (
            ...
            "NOT starburst, NOT exploding star), "
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v214_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v214] submitted pid={pid[:8]} denoise={DENOISE} ipa={IPA_WEIGHT} (v147 base, starburst-ban + no-readable-word)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v214_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v214] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v214_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v214_bat_logo_raw.jpg"
        ...
        print(f"  saved v214_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v214.jpg"
