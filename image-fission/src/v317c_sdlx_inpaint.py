"""
v317c: ARMED text-only fission via SDXL + KSampler(denoise=1.0) + VAEEncodeForInpaint.
Skip Fooocus patch (weights_only incompatible).
"""
import sys
import json
import time
...
from v316e_lib import prepare_text_mask, BANDS

SRC = r'E:/Desktop/双接口/image-fission/jobs/v315/source_armed.jpg'
OUT_DIR = r'E:/Desktop/双接口/image-fission/jobs/v317c'
...
print(f'[v317c] Source: {W}x{H}, fill_mask coverage: {fill_mask.sum()/fill_mask.size*100:.2f}%')

img_pil = Image.fromarray(img_rgb)
img_pil.save(os.path.join(OUT_DIR, 'source_clean.png'))
mask_pil = Image.fromarray((fill_mask * 255).astype(np.uint8), mode='L')
...
print(f'[v317c] Uploaded: img={img_name}, mask={mask_name}')

# Use CounterfeitXL or JuggernautXL SDXL + KSampler(denoise=1.0) + VAEEncodeForInpaint
prompt = {
    "1": {
    ...
    "5": {
        ...
        "inputs": {
            "clip": ["1", 1],
            "text": "military camouflage pattern, desert sand brown, gritty texture, fabric weave, repeating pattern, no text, no letters, no numbers",
        },
    },
    "6": {
        "class_type": "CLIPTextEncode",
        "inputs": {
            "clip": ["1", 1],
            "text": "text, watermark, logo, blurry, deformed, low quality",
        },
    },
    "7": {
...
print('[v317c] Submitting workflow...')
...
with urllib.request.urlopen(prompt_req, timeout=30) as resp:
    ...
    if 'error' in result:
        print(f'[v317c] ERROR: {json.dumps(result, indent=2)[:500]}')
        sys.exit(1)
    prompt_id = result['prompt_id']
print(f'[v317c] prompt_id={prompt_id}')

print('[v317c] Waiting (SDXL inpaint ~2-3 min)...')
history = None
...
print(f'[v317c] has_error: {has_error}')
if has_error:
    for m in status_msgs:
        if 'execution_error' in str(m[0]):
            print(f'  exception: {m[1].get("exception_type")}')
            print(f'  message: {m[1].get("exception_message", "")[:300]}')
            break
    sys.exit(1)
...
print(f'[v317c] Downloaded: {inpainted_path}')
...
if inpainted.shape != source_arr.shape:
    ...
    print(f'[v317c] Resized to {source_arr.shape[1]}x{source_arr.shape[0]}')
...
for band in BANDS:
    ...
    print(f'[v317c] {band["name"]}: text="{text}" font={fsize}px pos=({x},{y})')

final_path = os.path.join(OUT_DIR, 'armed_text_only_v317c.jpg')
out_pil.save(final_path, quality=95)
print(f'[v317c] FINAL: {final_path}')
...
combined.save(os.path.join(OUT_DIR, 'armed_compare_v317c.jpg'), quality=90)
print('[v317c] Done')