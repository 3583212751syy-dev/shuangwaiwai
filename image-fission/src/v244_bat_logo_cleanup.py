"""
v244 — bat_logo cleanup（v213 基线 + NEG 修月牙/3D 锥/雾晕/紫雾扩散/叠字/横条带）
...
策略：
  ...
  6. OCR 自检：不能有任何可读字
  7. 出图左右对照（要求差异显著：徽章转角、蝙蝠重画、小元素改、骨架改）

历史教训（v242/v243 用户打脸）：
  - v243 烧回 PIL 弧字后被 SDXL 二次覆盖 = 字叠字 + 紫雾化
  - v242 走"原图剪影旋转"伪创新，被批"跟原图没区别"
  - v244 砍掉烧字路径，回到 v213 真裂变最强稳态 7/10 + 修已知瑕疵
"""
...
SEED = 244001  # 与 v193 (700510)/v188 (700508)/v185 (700501)/v164 (700401) 不同
...
# ==================== Prompt 框架（基于 v193 bat_logo region, 强化差异化）====================
GLOBAL_POS = (
...
REGIONS = [
    # 中心徽章：保留徽章骨架，但蝙蝠重画、徽章转角、加装饰
    {
        ...
        "prompt": (
            ...
            "the emblem ring is twisted slightly with baroque flourishes, "
            "no ornamental moon or sphere inside the ring, no 3D pyramid or cone below, "
            "no beads or hanging pendants around the bat, no laurel wreath accent, "
            "no foggy halo or purple mist around the bat silhouette, "
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v244_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v244] submitted pid={pid[:8]} denoise={DENOISE} ipa={IPA_WEIGHT} (v147 base)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v244_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v244] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v244_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v244_bat_logo_raw.jpg"
        ...
        print(f"  saved v244_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v213.jpg"
