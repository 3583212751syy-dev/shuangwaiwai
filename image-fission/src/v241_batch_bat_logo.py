"""
v241 — 一次生成 3 个明显不同的蝙蝠姿态变体(扁平展翅 / 俯冲下扑 / 环抱内收)
...
MASKED_REF = "v241_masked_ref.png"
...
def make_masked_ref():
    ...
    print(f"[v241] masked ref saved (central filled purple {tuple(purple)})")
...
def build(seed, pose_global, pose_region, tag):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": f"v241_{tag}_bat_logo"}}
    return g
...
def gen(seed, pose_global, pose_region, tag):
    ...
    print(f"[v241-SDXL] pid={pid[:8]} seed={seed}")
    for _ in range(180):
        time.sleep(5)
        try:
            h = history(pid)
...
def main():
    ...
    for tag, pose_global, pose_region in POSES:
        ...
        if not raw:
            print(f"[v241] {tag} FAIL"); continue
        ...
        tmp = JOB / f"v241_{tag}_pre.jpg"; matched_rgb.save(str(tmp), quality=95)
        out = JOB / f"v241_{tag}.jpg"
        n = ocr_erase_text(str(tmp), str(out))
        side_by_side(COMFY_INPUT / REF_IMG, out, JOB / f"_compare_v241_{tag}.jpg")
        ...
        print(f"[v241] {tag}: erased={n} | 配色 hist={hi:.3f} | 结构差异={sd:.3f}")
        results[tag] = out
        print(f"  saved v241_{tag}.jpg")
    ...
    if results:
        ...
        grid.save(str(JOB / "_v241_grid.jpg"), quality=95)
        print(f"[v241] grid saved _v241_grid.jpg ({len(imgs)} cols)")
