"""
v229 — bat_logo 真裂变（v226 同源，修 v228 三大视觉缺陷）

v228 虽 QC 零红旗 PASS，但用户目检发现三大崩坏：
  A) 环上白色火焰/巴洛克卷草花纹 → 软糊无意义（v226 prompt 里 "thin clean minimal ring" 没拦 SDXL 加 baroque）
  B) 环下两只小展翅蝙蝠 + 中央宝石吊坠 + 浮空星星 → 意义不明小元素（SDXL 自加，原图无）
  D) 整体变成 3D 珠宝吊坠/项链 → 与原图 2D flat vintage 印刷徽章关联尽失（v214 已否 3D 项链死灰复燃）

v229 复制 v226 骨架，同 seed 受控，仅重构 prompt + 删多余 region：
  - 环 = 纯一条细线（plain ring LINE, NO baroque, NO flames, NO shapes on the ring）
  - 风格 = 强锁 2D flat 印刷（strictly 2D flat vector print, NO 3D, NO gem, NO jewelry, NO medallion, NO glossy）
  - 环下 = 彻底空（completely empty purple space, NO bats below, NO gems, NO pendants, NO floating objects）
  - 顶部 = 单只小细弯月（呼应 MURCIELAGO 哥特主题，不加星星宝石）
  - region 减至 1 = 只留中心徽章（删顶部弧带+底部饰带，SDXL 无空间自加元素）
  - 保留 v228 配色锁 + v226 技术栈（denoise 0.80 / lora 0.50 / ipa 0.18 / canny 0.25 / tile 0.60 / NMKD 4x / seed 220001）
目标：修回 2D flat vintage 徽章风，消除小元素与3D项链观感，同时维持 QC 干净 PASS。
"""
...
NEG_BASE = (
    ...
    # —— v229 新增：修 v228 三大崩坏（环上花纹糊/环下小元素/3D项链）——
    ...
    "shadow cast, depth, reflection, shading gradient, glossy surface, polished surface"
)
...
GLOBAL_POS = (
    "a perfectly centered 2D flat vintage circular purple emblem badge, "
    "a single thin clean plain circular ring outline ONLY — a simple thin ring LINE, "
    "NO baroque, NO scrollwork, NO flames, NO shapes on the ring, NO relief, NO decoration on the ring, NO pattern on the ring, "
    "ONE single stylized 2D bat silhouette inside the ring (NOT realistic, NOT 3D, NOT photographic, NOT gem, NOT jewelry), "
    "a single thin clean arched line following the top of the ring (NOT text, NOT words, NOT letters, NOT readable), "
    "completely empty purple space below the ring (NO bats below, NO gems below, NO pendants below, NO floating objects, NO small elements), "
    "STRICTLY 2D flat vector vintage craft spirits label print style, "
    "NO 3D rendering, NO glossy highlights, NO gem appearance, NO jewelry, NO pendant, NO necklace, NO medallion, NO chandelier, NO crystal, "
    "NO shadow, NO depth, NO reflection, NO shading gradient, "
    ...
    "sharp clean edges, bold emblem, symmetric composition, "
    "NO starburst, NO firework, NO photographic, NO readable text, NO readable word, NO multiple bats"
)
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v229_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v229] submitted pid={pid[:8]} denoise={DENOISE} lora={LORA_DETAIL} (v221 + cleaner composition, same seed, controlled)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v229_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v229] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v229_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v229_bat_logo_raw.jpg"
        ...
        print(f"  saved v229_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v229.jpg"
