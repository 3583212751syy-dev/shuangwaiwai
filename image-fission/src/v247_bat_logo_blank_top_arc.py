"""
v247 — bat_logo strict original style（按原图 BACARDÍ bat logo 风格/设计/排版严格做）
...
SEED = 247001  # 与 v246 (246001)/v245 (245001)/v244 (244001)/v213 (213001) 不同
...
NEG_BASE = (
    ...
    # v247 新增 — 禁所有字形 (SDXL 不要画任何字母/字符/字形, 字由 PIL 后期烧)
...
REGIONS = [
    ...
    # 顶部弧形带: v247 改成完全留白 (SDXL 不画任何东西, 装饰花纹 + 烧字全 PIL 后期)
    {
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v247_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v247] submitted pid={pid[:8]} denoise={DENOISE} ipa={IPA_WEIGHT} (strict original style + vector + post-burn)")
    for _ in range(120):
        time.sleep(5)
        try:
            h = history(pid)
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v247_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v247] NOT FOUND raw_path")
        return None
    return raw_path
...
# ==================== v247 新增: PIL 后期烧非侵权词 (main 末尾, 不经过 SDXL 不会糊) ====================
...
def main():
    out_final = JOB / "v247_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        try:
            ...
            print("  [v247] top arc burn OK: LVMEN NOCTIS")
        except Exception as e:
            print(f"  [v247] top arc burn FAIL: {e}")
        try:
            ...
            print("  [v247] bottom word burn OK: NOCTWING + MORS VINI")
        except Exception as e:
            print(f"  [v247] bottom word burn FAIL: {e}")
        try:
            burn_est_year(final_img, int(final_img.height * 0.022))
            print("  [v247] est year burn OK: Est. 1862")
        except Exception as e:
            print(f"  [v247] est year burn FAIL: {e}")
        ...
        print(f"  saved v247_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v247.jpg"
