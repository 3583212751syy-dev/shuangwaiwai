"""
v242 — 一次生成 3 个明显不同的蝙蝠姿态变体(扁平展翅 / 俯冲下扑 / 环抱内收)
...
MASKED_REF = "v242_masked_ref.png"
...
# 四段姿态(明显换角度+换体态, 仍同源蝙蝠物种/向上感/2D平面紫锁): (tag, GLOBAL_POS 蝙蝠行, REGION[0] 蝙蝠行)
POSES = [
    ("a_bank",
     "the bat is BANKING sharply and ASYMMETRICALLY to its RIGHT in mid-flight - its entire body is ROTATED roughly 35 degrees, the LEFT wing is swept HIGH upward while the RIGHT wing is dipped LOW, the head is turned to glance back over its shoulder toward the viewer; a dynamic turning flying pose, clearly NOT a symmetric front-facing bat, but unmistakably the SAME bat species and clearly related to the source bat, NOT inverted, NOT drooping, NOT a copy of the source pose, "
     ...
     "the bat is in a NEW dynamic pose: BANKING sharply to its RIGHT, body rotated ~35 degrees, LEFT wing up HIGH and RIGHT wing dipped LOW, head turned glancing back - a related VARIANT pose of the source heraldic bat, NOT a flat front-on symmetric bat, NOT a copy of the source, NOT inverted, NOT drooping, "
    ...
    ("b_dive",
     "the bat is PLUNGING DOWNWARD in a dramatic dive - its body is VERTICAL with the HEAD pointing DOWN toward the bottom of the frame, both wings swept UP and BACK into a narrow upward V behind the body, hind legs trailing below; a dynamic diving pose, clearly different from a horizontal flying bat, but unmistakably the SAME bat species and clearly related to the source bat, NOT an upside-down inversion (head-down dive is intentional motion), NOT drooping, NOT a copy of the source pose, "
     "the bat is a CLEAN smooth solid silhouette with NO internal lines, NO letter-like curves, NO scrollwork, NO patterns inside, smooth wing membranes only, ",
     "the bat is in a NEW pose: PLUNGING DOWN in a dive, body vertical head-down, wings swept up and back into a narrow V, legs trailing - a related VARIANT pose of the source heraldic bat, NOT a flat front-on symmetric bat, NOT a copy of the source; "
     "the bat is a CLEAN smooth solid black silhouette, NO internal detail lines, NO letter-like shapes on the wings, NO scrollwork inside the silhouette, smooth wing membranes, "),
    ("c_curl",
     "the bat has its wings CURLING FORWARD and INWARD in a protective embrace arc that wraps around its body, body compact and UPRIGHT, head tilted UP looking skyward, wingtips nearly meeting below the head; a curled compact pose, clearly different from a spread-wing bat, but unmistakably the SAME bat species and clearly related to the source bat, NOT inverted, NOT drooping, NOT a copy of the source pose, "
     "the bat is a CLEAN smooth solid silhouette with NO internal lines, NO letter-like curves, NO scrollwork, NO patterns inside, smooth wing membranes only, ",
     "the bat is in a NEW pose: wings curling FORWARD and INWARD in an embrace arc, body compact upright, head tilted up, wingtips nearing below the head - a related VARIANT pose of the source heraldic bat, NOT a flat front-on symmetric bat, NOT a copy of the source, NOT inverted, NOT drooping; "
     "the bat is a CLEAN smooth solid black silhouette, NO internal detail lines, NO letter-like shapes on the wings, NO scrollwork inside the silhouette, smooth wing membranes, "),
    ("d_top",
     "the bat is viewed from a 3/4 TOP-DOWN aerial angle as if flying just above the viewer - its body is FORESHORTENED and pointing toward the viewer (downward in frame), wings spread WIDE but seen at a tilted perspective, head facing the viewer; an unusual aerial perspective pose, clearly different from a flat front-facing bat, but unmistakably the SAME bat species and clearly related to the source bat, NOT inverted, NOT drooping, NOT a copy of the source pose, "
     "the bat is a CLEAN smooth solid silhouette with NO internal lines, NO letter-like curves, NO scrollwork, NO patterns inside, smooth wing membranes only, ",
     "the bat is in a NEW pose: viewed from a 3/4 TOP-DOWN aerial angle, body foreshortened pointing toward viewer, wings spread wide at a tilted perspective, head facing viewer - a related VARIANT pose of the source heraldic bat, NOT a flat front-on symmetric bat, NOT a copy of the source, NOT inverted, NOT drooping; "
     "the bat is a CLEAN smooth solid black silhouette, NO internal detail lines, NO letter-like shapes on the wings, NO scrollwork inside the silhouette, smooth wing membranes, "),
]

SEEDS = {"a_bank": 241001, "b_dive": 241002, "c_curl": 241003, "d_top": 241004}

# 从原图提取真实蝙蝠剪影, 用仿射变换换角度/体态, 作为 Canny 姿态硬约束
# (rot=度数, flip_h/flip_v=镜像, scale=缩放) —— 同源物种、画风材质不变, 只换角度
BAT_BOX = (0.28, 0.16, 0.52, 0.46)   # 提取蝙蝠本体的内框(避开顶部弧线文字)
POSE_XFORM = {
    "a_bank": (35, True, False, 1.00),    # 向右急转倾斜
    "b_dive": (72, False, False, 1.06),   # 侧俯冲(头甩向一侧)
    "c_curl": (18, False, False, 0.82),   # 紧凑收拢(缩小+微旋)
    "d_top":  (-22, True, False, 1.12),   # 镜像+微倾(顶视俯瞰感)
}
...
def make_masked_ref():
    ...
    print(f"[v242] masked ref saved (central filled purple {tuple(purple)})")
...
def extract_bat_mask():
    ...
    ys, xs = np.where(full)
    if len(xs) == 0:
        raise RuntimeError("[v242] bat mask empty - 阈值可能失效")
    print(f"[v242] bat mask: {len(xs)}px bbox=({xs.min()},{ys.min()})-({xs.max()},{ys.max()})")
    return full
...
def make_pose_refs():
    ...
    for tag, (rot, fh, fv, sc) in POSE_XFORM.items():
        ...
        out.save(str(COMFY_INPUT / f"v242_pose_{tag}.png"), "PNG")
        print(f"[v242] pose ref saved v242_pose_{tag}.png (rot={rot} fh={fh} fv={fv} sc={sc})")
...
def build(seed, pose_global, pose_region, tag):
    ...
    g["3b"] = {"class_type": "LoadImage", "inputs": {"image": f"v242_pose_{tag}.png"}}
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": f"v242_{tag}_bat_logo"}}
    return g
...
def gen(seed, pose_global, pose_region, tag):
    ...
    print(f"[v242-SDXL] pid={pid[:8]} seed={seed}")
    for _ in range(180):
        time.sleep(5)
        try:
            h = history(pid)
...
def main():
    ...
    for tag, pose_global, pose_region in POSES:
        ...
        if not raw:
            print(f"[v242] {tag} FAIL"); continue
        ...
        tmp = JOB / f"v242_{tag}_pre.jpg"; matched_rgb.save(str(tmp), quality=95)
        out = JOB / f"v242_{tag}.jpg"
        n = ocr_erase_text(str(tmp), str(out))
        side_by_side(COMFY_INPUT / REF_IMG, out, JOB / f"_compare_v242_{tag}.jpg")
        ...
        print(f"[v242] {tag}: erased={n} | 配色 hist={hi:.3f} | 结构差异={sd:.3f}")
        results[tag] = out
        print(f"  saved v242_{tag}.jpg")

    # 4 联对照: 原图 + 三变体
    if results:
        orig = Image.open(COMFY_INPUT / REF_IMG).convert("RGB")
        imgs = [orig] + [Image.open(results[t]).convert("RGB").resize(orig.size) for t in ["a_bank", "b_dive", "c_curl", "d_top"] if t in results]
        gap = 20; W = orig.width * len(imgs) + gap * (len(imgs) - 1)
        grid = Image.new("RGB", (W, orig.height), "white")
        x = 0
        for im in imgs:
            grid.paste(im, (x, 0)); x += orig.width + gap
        grid.save(str(JOB / "_v242_grid.jpg"), quality=95)
        print(f"[v242] grid saved _v242_grid.jpg ({len(imgs)} cols)")
