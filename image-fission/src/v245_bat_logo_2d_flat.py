"""
v245 — bat_logo 2D flat（v244 基线 + 项链/吊坠/缎带/金属浮雕 强 NEG 拉回 2D craft label）

目标：BACARDÍ bat_logo (test_6978fabda2cc99629fa9e81f802762d3.jpg)
策略：
  1. v147 锁基线参数 (denoise 0.80 / ipa 0.18 / lora 1.0 / canny 0.25 / tile 0.60)
  2. 字作为装饰图形元素一起重画，绝不 PIL 烧可读字（v213 立的铁律#1，最高优先级）
  3. 3 region: top arc / main badge / bottom banner
  4. 后置 Reinhard LAB 配色迁移（紫底锁定）
  5. NEG 加大量禁项：月牙球/3D 锥/紫雾/inpaint 漂白/叠字/横条带
     + v245 新增：项链/吊坠/缎带/金属浮雕/3D 珠宝（v244 漂成 necklace pendant 修了）
  6. GLOBAL_POS 强拉 flat 2D printed craft label, no 3D, no jewelry, no necklace
  7. REGIONS 减 baroque 浮雕 → subtle clean linework；禁 no ribbon/no pendant/no silver
  8. OCR 自检：不能有任何可读字
  9. AI 肉眼看图自检（v244+ 红线：OCR PASS 不算 PASS，必须肉眼审）

历史教训（v242/v243/v244 用户打脸）：
  - v243 烧回 PIL 弧字后被 SDXL 二次覆盖 = 字叠字 + 紫雾化
  - v242 走"原图剪影旋转"伪创新，被批"跟原图没区别"
  - v244 砍掉烧字路径，但画风漂到 3D 银质珠宝项链（顶部缎带+底部水滴宝石吊坠）
  - v245 强拉 2D flat + 强禁 necklace/pendant/draped cloth/metallic 3D
"""
...
SEED = 245001  # 与 v244 (244001)/v213 (213001) 不同
...
# ==================== NEG: 禁可读字+禁原样复制+禁漂色+禁月牙/3D/雾+禁项链/吊坠/缎带/金属 ====================
NEG_BASE = (
    ...
    # v245 强禁：项链/吊坠/缎带/金属浮雕/3D 珠宝（v244 漂到 necklace pendant 修了）
...
# ==================== Prompt 框架（基于 v244 bat_logo region, v245 强拉 2D flat）====================
GLOBAL_POS = (
    "vintage gothic purple spirit brand emblem badge centered on a soft purple background, "
    "purple #6B2C8C and deep violet #2A0A3F and black #1A0A1F color blocks only, "
    "a stylistically transformed circular emblem with an evolved bat silhouette in the center, "
    "decorative ornamental STYLIZED LETTERING shaped into the badge design as graphical elements "
    "(abstract letter-shaped ribbons and curlicues woven into the badge, NOT readable English words, "
    "NOT spelled-out words, NOT any brand name), "
    "the emblem ring is rotated 18 degrees and slightly distorted with subtle clean linework, "
    "vintage craft spirits label art style, sharp clean edges, bold emblem, "
    "no sphere or moon above the emblem ring, no 3D pyramid or cone below the badge, "
    "no foggy purple haze around the silhouette, "
    # v245 强拉 2D flat craft label（修 v244 漂 3D 银质珠宝项链）
    "flat 2D printed graphic, vintage printed paper label art, screen print aesthetic, "
    "no 3D rendering, no metallic luster, no jewelry aesthetic, no necklace, no pendant, "
    "no ribbon crossing, no draped fabric, sharp clean edges, no chrome, no silver relief"
)

REGIONS = [
    # 中心徽章：保留徽章骨架，但蝙蝠重画、徽章转角、加装饰
    {
        "x": 0.05, "y": 0.10, "w": 0.90, "h": 0.80, "strength": 1.30,
        "prompt": (
            "a stylistically transformed circular purple emblem with an evolved bat silhouette inside, "
            "the bat is rotated 32 degrees clockwise, wings spread 1.25x wider with sharper talons, "
            ...
            "NOT any brand name, looks like artistic letterform patterns integrated into the design), "
            "the emblem ring is twisted slightly with subtle clean linework (not heavy baroque, not metallic 3D relief), "
            "no ornamental moon or sphere inside the ring, no 3D pyramid or cone below, "
            "no beads or hanging pendants around the bat, no laurel wreath accent, no necklace chain, no gemstone drop below, "
            "no foggy halo or purple mist around the bat silhouette, no silver metallic relief, no chrome luster, "
    ...
    # 顶部弧形带：装饰性图案带
    {
        ...
        "prompt": (
            ...
            "(no ribbon crossing, no draped fabric, no chain), "
            ...
            "flat 2D purple and black color blocks with sharp edges, no metallic 3D relief. " + COHESIVE
        )
    },
    # 底部徽章饰带：装饰性图案
    {
        ...
        "prompt": (
            "a flat bottom decorative banner (no hanging pendant, no gemstone drop, no chain) "
            "with abstract ornamental patterns and stylized "
            ...
            "kept flat 2D purple and black palette, no 3D silver, no metallic luster. " + COHESIVE
        )
    },
]
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v245_bat_logo"}}
    return g


# ==================== 后置: Reinhard LAB 配色迁移 ====================
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v245] submitted pid={pid[:8]} denoise={DENOISE} ipa={IPA_WEIGHT} (v244 base + 2D flat)")
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v245_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v245] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v245_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v245_bat_logo_raw.jpg"
        ...
        print(f"  saved v245_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    # 对照拼图
    out_cmp = JOB / "_compare_v245.jpg"
