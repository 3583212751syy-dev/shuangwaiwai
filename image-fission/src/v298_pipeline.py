...
SUBJECT_VARIANTS = [
    ("up",     "NOCTAVEN", "DISTILLERY", "SHADOW OF THE WING",      298001),
    ("spread", "DUSKBAT",  "RESERVE",    "WINGS OF TWILIGHT",       298101),
    ("fold",   "MOONBAT",  "NOCTURNE",   "GUARDIAN OF THE DARK",    298201),
]
...
def make_clean_source():
    ...
    Image.fromarray(debug_img).save(r"E:/Desktop/双接口/image-fission/jobs/v298/_debug_mask.png")
    ...
    # 5) y > 940（蝙蝠尾巴以下）分区清除：
    #    圆盘外非蝙蝠 → 全填紫底；圆盘内非蝙蝠 → 只清黑白字（保留原图紫底连续性）
    ...
    dark_or_bright = (arr.min(axis=2) < 80) | (arr.min(axis=2) > 215)
    clear_inside = lower_zone & not_bat & inner_badge_bool & dark_or_bright
    clean[clear_outside | clear_inside] = bg_color
    print(f"[lower] 盘外清 {int(clear_outside.sum())}px 盘内字清 {int(clear_inside.sum())}px")

    # 6) 圆盘内非蝙蝠暗像素的孤立小连通域（杂点）删除
    ...
    for i in range(1, n_lab):
        if stats[i, cv2.CC_STAT_AREA] < 600:
            clean[labels == i] = bg_color
            removed += 1
    print(f"[stray] 删除小杂点 {removed} 个")
...
def main():
    ...
    src_path = COMFY_INPUT / "v298_text_free_source.png"
    ...
    if not src_path.exists() or args.force:
        ...
        print(f"[v298] clean source -> {src_path}")
    else:
        print(f"[v298] reuse clean source")
    ...
    for tag, big, sub, arc, _ in SUBJECT_VARIANTS:
        ...
        composed.save(JOB / f"v298_{tag}_composed.png", quality=95)
        ...
        final_path = JOB / f"v298_{tag}_final.png"
        final.save(final_path, quality=95)
        files.append(final_path)
        print(f"[v298] {tag} final -> {final_path.name}")
    ...
    gp = JOB / "_grid_v298.png"
    grid.save(gp, quality=92)
    print(f"[v298] grid -> {gp}")
