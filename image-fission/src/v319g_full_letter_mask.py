"""v319g: LaMa mask = letter CC & gray<160 (full anti-aliased letter bodies).
Complete text removal, accepting LaMa edge haze as inevitable model limitation.

vs v319f (failure): mask was only gray<80 letter cores. Anti-aliased edges
  (gray 80-160) left visible ARMED/FORCES ghost. User screenshot showed this
  as 'word shadow + haze band'.

v319g mask = (letter CC body) & (gray < 160). This covers the entire letter
  including anti-alias. LaMa will completely remove the text, but will leave
  ~2px haze band around the mask (Fourier conv limitation, can't be avoided).
  The new text (Arial Black, fsize=138, bottom-aligned to y1) overwrites the
  haze band inside the text band.

  Chain + dog tag copy-back from source still applied.
"""
import cv2
import numpy as np
import shutil
...
OUT_DIR = str(PROJECT / "jobs" / "v319g")
...
for band in BANDS_DEF:
    ...
    for i in range(1, n):
        ...
        if 8000 < a < 30000 and h > 80 and 0.5 < w/h < 1.5:
            ...
            print(f'[v319g] {band["name"]}: dog-tag CC area={a} w={w} h={h}')

# Step 2: mask = letter CC body & gray<160 (full letter including anti-alias)
text_mask = letter_mask & (gray < 160) & (chain_mask == 0) & (dog_tag_mask == 0)
print(f'[v319g] text mask px (gray<160 letter bodies): {int(text_mask.sum())}')
...
print('[v319g] LaMa inpaint (mask=gray<160 letter bodies, no dilate, edge=1)...')
cleaned = lama_inpaint(src_pil, mask_pil, removal_strength=230, edge_smoothness=1)

# Step 3: defensive copy-back for chain + dog tag
...
for b in BANDS_DEF:
    ...
    print(f'[v319g] {b["name"]}: capH={capH} fsize={fsize} "{text}" nat_w={nat_w} spacing={spacing} '
...
OUT = Path(OUT_DIR) / 'armed_text_only_v319g.jpg'
...
combined.save(str(Path(OUT_DIR) / 'armed_compare_v319g.jpg'), quality=90)
...
band_compare.save(str(Path(OUT_DIR) / 'armed_text_band_compare_v319g.jpg'), quality=95)
print(f'[v319g] Saved {OUT}')