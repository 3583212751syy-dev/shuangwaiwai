"""v135: 锁结构裂变正解 (v128 单步管线 + denoise 0.62 + 无 Stage0 预清字).

根因复盘 (v133/v134 失败):
  Stage0 预清字用 inpaint denoise 0.92 抹字, 把整图结构也重画了
  (clean 中间产物 SSIM 仅 0.07~0.30). v128 不跑 Stage0, 结构完好 (SSIM 0.78~0.96).

v135 方案:
  - 不做 Stage0 预清字 (原图直接进裂变)
  - Canny 0.55 锁构图骨架 + IPAdapter style 0.55 锁配色
  - denoise 0.62 (v128 的 0.55 略提升, 给内容更多裂变空间, 仍保结构)
  - 二阶 KSampler 细化 (denoise 0.18) 降噪
  - 文字: 后期 PIL 烧 (确定性拼写, 按原图位置, 图相关词)
"""
...
REFS = [
    {"id": "illust_1", "filename": "pinterest_illust_1.jpg", "weight": 0.50, "cn_weight": 0.45,
    ...
    {"id": "eagle_2",  "filename": "pinterest_eagle_2.jpg",  "weight": 0.50, "cn_weight": 0.45,
    ...
    {"id": "denim_3",  "filename": "pinterest_denim_3.jpg",  "weight": 0.50, "cn_weight": 0.45,
    ...
    {"id": "camo_4",   "filename": "pinterest_camo_4.jpg",   "weight": 0.50, "cn_weight": 0.45,
    ...
    {"id": "skull_5",  "filename": "pinterest_skull_5.jpg",  "weight": 0.50, "cn_weight": 0.45,
    ...
    {"id": "metal_6",  "filename": "pinterest_metal_6.jpg",  "weight": 0.50, "cn_weight": 0.45,
...
def build_graph(ref):
    ...
    g["17"] = {"class_type": "SaveImage", "inputs": {"images": ["16", 0], "filename_prefix": "v136_clean"}}
    return g
...
def run_one(ref, out_dir):
    import requests
    tag = ref["id"]
    out_path = out_dir / f"v136_clean_{tag}.png"
    if out_path.exists() and out_path.stat().st_size > 100_000:
        print(f"[skip] {tag}: 已存在 {out_path.stat().st_size//1024}KB", flush=True); return True
    print(f"[run] {tag}  cn={ref['cn_weight']}  ipa={ref['weight']}  denoise=0.68", flush=True)
    g = build_graph(ref)
    try:
        r = requests.post(f"{COMFYUI}/prompt", json={"prompt": g, "client_id": f"v135_{int(time.time())}"}, timeout=15)
...
def main():
    out = PROJECT_ROOT / "jobs" / f"smoke_v136_{int(time.time())}"
    out.mkdir(parents=True, exist_ok=True)
    print(f"=== v136 (Canny cn=0.55 + IPA style=0.55 + denoise=0.68, 无Stage0) -> {out} ===", flush=True)
    ok = 0
    for ref in REFS:
        if run_one(ref, out):
            ok += 1
    print(f"\n=== done v135: {ok}/{len(REFS)} ===", flush=True)
