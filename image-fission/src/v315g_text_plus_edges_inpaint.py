"""
v315g_text_plus_edges_inpaint.py
Build text mask = text pixels (gray<75) + dilated to capture anti-aliased edges.
Chain pixels excluded via CC filter so they're never inpainted.

Strategy:
  1. Find text CCs (wide+tall, not small chain links) per band
  2. Dilate text mask 6-8px to capture anti-aliased edges
  3. Subtract chain pixels from dilated mask (chain never touched)
  4. TELEA inpaint with small radius (3) — preserves local camo texture
  5. Write new text
"""
import cv2
import numpy as np
...
print(f'[v315g] Loaded {W}x{H}')

BANDS = [
    {'name': 'small', 'y0': 472, 'y1': 523, 'min_h_chain': 25, 'min_w_chain': 12, 'min_area_chain': 200, 'x_pad': 30, 'dilate_px': 6},
    {'name': 'big1',  'y0': 541, 'y1': 640, 'min_h_chain': 60, 'min_w_chain': 30, 'min_area_chain': 1000, 'x_pad': 30, 'dilate_px': 10},
    {'name': 'big2',  'y0': 656, 'y1': 755, 'min_h_chain': 60, 'min_w_chain': 30, 'min_area_chain': 1000, 'x_pad': 30, 'dilate_px': 10},
]

# Build text mask (text pixels only, no dilation yet)
...
print(f'[v315g] text_only_mask pixels: {int((text_only_mask>0).sum())}')
print(f'[v315g] chain_mask pixels: {int((chain_mask>0).sum())}')

# Dilate text mask per band (different dilate sizes)
...
for band in BANDS:
    ...
    if band_text.sum() == 0:
        continue
    dilated = cv2.dilate(band_text, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2*dilate_px+1, 2*dilate_px+1)), iterations=1)
    # Subtract chain from dilated
...
print(f'[v315g] fill_mask pixels: {int((fill_mask>0).sum())}')

# Save fill mask for inspection
mask_img = (fill_mask).astype(np.uint8)
Image.fromarray(mask_img).save(os.path.join(OUT_DIR, 'mask_v315g.png'))

# Inpaint with TELEA small radius
...
print('[v315g] Inpaint TELEA r3 done')
...
for band in BANDS:
    ...
    print(f'[v315g] Band {band["name"]}: text="{text}" font={fsize}px '
          f'pos=({x},{y}) orig_cx={orig_cx}')

OUT = os.path.join(OUT_DIR, 'armed_text_only_v315g.jpg')
out_pil.save(OUT, quality=95)
print(f'[v315g] Saved {OUT}')
...
combined.save(os.path.join(OUT_DIR, 'armed_compare_v315g.jpg'), quality=90)
print('[v315g] Saved compare')