"""v171 eagle_2 大裂变样张（连主体也换姿态 + 装饰元素大换）

目标：把 fission 幅度明显推大，解决用户反馈「内容变化不明显」。
相对 v164 的改动：
- DENOISE 0.80 -> 0.88（内容重画空间更大）
- TILE 0.60 -> 0.45（降低纹理锁定，放更多新细节进来）
- CANNY 0.25 -> 0.20（保留整体布局/位置，但不锁死主体姿态，允许换姿态）
- REGION_STRENGTH_SCALE 0.55 -> 0.85（区域提示主导性更强，新母题渲染更清晰）
- IPA 0.18 保持（只锁画风/配色）
- eagle_2 区域提示全部重写：
  主体鹰 = 3/4 侧脸 + 收翅 + 抓破齿轮（姿态大改，物种不变）
  骷髅 -> 熔岩裂缝黑曜石
  闪电 -> 白色水晶簇
  蛇 -> 钢链框
构图（区域位置）不变，内容裂变明显。

输出：outputs/v171/v171_eagle_2.jpg + 对照拼图 v171_eagle_2_compare.jpg（原图|裂变）
"""
...
REFS = [
    # === eagle_2（v171 大裂变样张）===
    {
        ...
        "regions": [
            # 主体：鹰（连主体也换姿态——3/4 侧脸 + 收翅 + 抓破齿轮）
            {"x": 0.30, "y": 0.00, "w": 0.40, "h": 0.50, "strength": 1.35,
             "prompt": ("ONE large bald eagle in a dramatic 3/4 SIDE PROFILE pose, "
                        ...
                        "fierce intense profile silhouette. " + COHESIVE)},
            # 原骷髅位置 -> 熔岩裂缝黑曜石（装饰大换）
            {"x": 0.25, "y": 0.45, "w": 0.50, "h": 0.40, "strength": 1.30,
             "prompt": ("a massive cracked OBSIDIAN monolith with glowing molten LAVA veins "
            ...
            {"x": 0.00, "y": 0.15, "w": 0.25, "h": 0.75, "strength": 1.15,
             "prompt": ("a burst of 7 sharp WHITE CRYSTAL QUARTZ shards fanning out from "
                        "the eagle's left shoulder, "
            ...
            {"x": 0.75, "y": 0.15, "w": 0.25, "h": 0.75, "strength": 1.15,
             "prompt": ("a MIRROR burst of 7 sharp WHITE CRYSTAL QUARTZ shards on the right side, "
...
def build(ref, seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": f"v171_{ref['id']}"}}
    return g


def gen(ref, seed, out_base):
    tag = ref["id"]
    out = out_base / f"v171_{tag}.jpg"
    ...
    g = build(ref, seed)
    r = requests.post(f"{COMFYUI}/prompt", json={"prompt": g, "client_id": f"v171_{int(time.time())}"}, timeout=15)
    try:
        j = r.json()
    except Exception:
        j = {}
...
def make_compare(orig_path, new_path, out_path):
    from PIL import Image, ImageDraw, ImageFont
    a = Image.open(orig_path).convert("RGB")
    b = Image.open(new_path).convert("RGB")
    H = 1024
    aw, ah = a.size; bw, bh = b.size
    a2 = a.resize((int(aw * H / ah), H))
    b2 = b.resize((int(bw * H / bh), H))
    W = a2.width + b2.width + 30
    canvas = Image.new("RGB", (W, H + 60), (20, 20, 20))
    canvas.paste(a2, (0, 60))
    canvas.paste(b2, (a2.width + 30, 60))
    d = ImageDraw.Draw(canvas)
    try:
        f = ImageFont.truetype("arial.ttf", 28)
    except Exception:
        f = ImageFont.load_default()
    d.text((a2.width // 2 - 40, 16), "原图", fill=(220, 220, 220), font=f)
    d.text((a2.width + 30 + b2.width // 2 - 40, 16), "v171 大裂变", fill=(220, 220, 220), font=f)
    canvas.save(out_path)
    print(f"  对照图 -> {out_path}", flush=True)


if __name__ == "__main__":
    out_base = PROJECT_ROOT / "outputs" / "v171"
    out_base.mkdir(parents=True, exist_ok=True)
    ref = REFS[0]
    ok = gen(ref, SEED, out_base)
    if ok:
        orig = PROJECT_ROOT / "ComfyUI" / "input" / ref["ref_img"]
        new = out_base / f"v171_{ref['id']}.jpg"
        if orig.exists() and new.exists():
            make_compare(orig, new, out_base / f"v171_{ref['id']}_compare.jpg")
