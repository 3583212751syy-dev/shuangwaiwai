"""
v292 — 修复 v291 主字/副字/EST.1862 全丢失的 bug。
...
SUBJECT_VARIANTS = [
    ("up",     "NOCTAVEN", "DISTILLERY", "SHADOW OF THE WING",      292001),
    ("spread", "DUSKBAT",  "RESERVE",    "WINGS OF TWILIGHT",       292101),
    ("fold",   "MOONBAT",  "NOCTURNE",   "GUARDIAN OF THE DARK",    292201),
]
...
def make_clean_source():
    ...
    # 1a) 在环带内把"近黑 OR 近白"像素（描边+字身）填成紫色，去掉 BACARDÍ / LA CASA 字
    # 直接用 in_band 减去 bat_protect 区域
    in_band = arc_band_keep.copy()
    in_band[bat_protect] = False
    black_or_white = ((arr.min(axis=2) < 60) | (arr.min(axis=2) > 230)) & in_band
    clean[black_or_white] = bg_color
    print(f"[arc] arc_band={int(arc_band.sum())} inner={int(inner.sum())} arc_band_keep={int(arc_band_keep.sum())} bat_protect={int(bat_protect.sum())} inter={int((arc_band_keep & bat_protect).sum())} in_band={int(in_band.sum())} black_or_white={int(black_or_white.sum())}")
    # debug vis
    ...
    Image.fromarray(debug_img).save(r"E:/Desktop/双接口/image-fission/jobs/v292/_debug_mask.png")
...
def main():
    ...
    src_path = COMFY_INPUT / "v292_text_free_source.png"
    ...
    if not src_path.exists() or args.force:
        ...
        print(f"[v292] clean source -> {src_path}")
    else:
        print(f"[v292] reuse clean source")
    ...
    for tag, big, sub, arc, _ in SUBJECT_VARIANTS:
        ...
        composed.save(JOB / f"v292_{tag}_composed.png", quality=95)
        ...
        final_path = JOB / f"v292_{tag}_final.png"
        final.save(final_path, quality=95)
        files.append(final_path)
        print(f"[v292] {tag} final -> {final_path.name}")
    ...
    gp = JOB / "_grid_v292.png"
    grid.save(gp, quality=92)
    print(f"[v292] grid -> {gp}")
