"""
v234 — v232 + v233 全知识:根治徽章外字符崩坏

OCR 实证 v232 / v233 崩坏:
  v232: ['7', 'HAGARIA', '(', 'S1UIEZI']
  v233: ['BAT', 'I', 'EN', 'MENEAR ]', '2']  ← 中央徽章外出现 "BAT"/"EN"/"MENEAR I"

根因：SDXL 看见 "vintage craft spirits label" + "circular emblem" → 自然联想到
"vintage label 通常带有 decorative banner text"，强迫 SDXL 在徽章外加假字母。

v234 根治策略：
  1. POS 直接否定 "decorative banner"，强调 "no text on the banner" 多次重复
  2. POS 顶部弧带改为 "a thin clean curved line"（不是 banner）
  3. 中央徽章 prompt 强化：clean ring ONLY + bat ONLY + NOTHING ELSE（无装饰空间）
  4. 加 OCR 已知崩坏字符到 NEG（hash 拦截）
  5. CFG 仍 8.0（v233 已 PASS 量化 QC）
  6. 维持 canny 0.65 + tile 0.95 + IPA 0.30 锁定结构
"""
...
CFG = 8.0  # v233 已证 PASS

# v234 NEG: v232 + v233 已知崩坏字符加入(防 hash)
NEG_BASE = (
    ...
    "any individual character shapes resembling BAT, BATMAN, MENEARI, MENAIR, MENERI, MENEAR, EN, ES, BAT EN, BAT EN, "
...
# v234 关键 prompt 改动:顶部弧带 = 一条细线(不是 banner)
GLOBAL_POS = (
    ...
    "the ONLY shapes present are: a thin curving line (above), a thin ring outline (badge), a stylized bat (badge), a small crescent (badge), empty purple space (everywhere else)"
)
...
def build(seed):
    ...
    g["15"] = {"class_type": "SaveImage", "inputs": {"images": ["14", 0], "filename_prefix": "v234_bat_logo"}}
    return g
...
def gen(seed):
    ...
    r = submit(g)
    pid = r["prompt_id"]
    print(f"[v234] submitted pid={pid[:8]} cfg={CFG} canny={CANNY_STRENGTH} (v233 + 顶部thin line + hash字符禁)")
    for _ in range(120):
        time.sleep(5)
        try:
            h = history(pid)
    ...
    for node_id, info in outputs.items():
        ...
        if raw_path:
            break
    if not raw_path:
        cands = sorted(COMFY_OUTPUT.glob("v234_bat_logo*.png"), key=lambda p: p.stat().st_mtime, reverse=True)
        if cands:
            raw_path = str(cands[0])
    if not raw_path:
        print(f"[v234] NOT FOUND raw_path")
        return None
    return raw_path
...
def main():
    out_final = JOB / "v234_bat_logo.jpg"
    if out_final.exists() and out_final.stat().st_size > 100000:
        ...
        out_raw = JOB / "v234_bat_logo_raw.jpg"
        ...
        print(f"  saved v234_bat_logo.jpg ({out_final.stat().st_size//1024} KB)")
        print(f"    配色: 迁移前={hi_before:.3f} → 迁移后={hi_after:.3f} | 结构差异={sd:.3f}")

    out_cmp = JOB / "_compare_v234.jpg"
