"""v128 v2: 按原图文字位置的软遮罩+烧字，不用黑方块.
- 遮罩颜色从 mask 区域边缘采样（融入背景），不再是纯黑
- 烧字位置按原图 OCR 实际区间
"""
import numpy as np
...
SPECS = {
    "eagle_2": {
        # 原图文字：中(0.55-0.61 "JACKE DANNIES") + 下(0.85-0.92 "IoAnDUJEL")
        "masks": [
            (0.28, 0.50, 0.72, 0.64),  # 中部文字带 (JACKE DANNIES 区域)
            (0.15, 0.82, 0.85, 0.94),  # 底部文字带 (IoAnDUJEL 区域)
            (0.35, 0.42, 0.65, 0.50),  # 中徽章额外AI字（柔化，不烧字）
        ],
        "burns": [
            {"word": "DOMINION", "font": "pirata_one", "size_ratio": 0.055, "y_ratio": 0.555,
             "fill": (245, 230, 215, 255), "stroke": (20, 12, 8, 255), "stroke_w": 3, "spacing": 3},
            {"word": "DOMINION", "font": "pirata_one", "size_ratio": 0.045, "y_ratio": 0.86,
    ...
    "denim_3": {
        ...
        "burns": [
            {"word": "UPCY", "font": "rye", "size_ratio": 0.14, "y_ratio": 0.09,
    ...
    "skull_5": {
        ...
        "burns": [
            {"word": "VENOM", "font": "pirata_one", "size_ratio": 0.085, "y_ratio": 0.08,
             "fill": (215, 30, 40, 255), "stroke": (8, 8, 12, 255), "stroke_w": 4, "spacing": 5},
            {"word": "VENOM", "font": "pirata_one", "size_ratio": 0.07, "y_ratio": 0.78,
             "fill": (215, 30, 40, 255), "stroke": (8, 8, 12, 255), "stroke_w": 3, "spacing": 4},
        ],
    },
    "metal_6": {
        # 原图文字：上(0.08-0.22 logo "W")
        "masks": [
            (0.10, 0.05, 0.90, 0.25),
            (0.53, 0.05, 0.79, 0.21),  # 右侧 681 残留
        ],
        "burns": [
            {"word": "MRCHGSR", "font": "metal_mania", "size_ratio": 0.115, "y_ratio": 0.06,
...
def process(key):
    ...
    for mask_spec in spec.get("masks", []):
        mask_layer = soft_mask_layer(im, *mask_spec, blur=20, alpha=210)
...
def main():
    print("=== v128 v2: 软遮罩(边缘采样色) + 按原图位置烧字 ===", flush=True)
