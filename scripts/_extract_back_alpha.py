...
# Distance to white background
dist_to_white = np.sqrt(np.sum((255.0 - rgb) ** 2, axis=2))

# Soft alpha based on distance from white
# Foreground pixels: alpha increases with distance from white
alpha = np.clip(dist_to_white * 2.5, 0, 255).astype(np.uint8)
# But only within hard_mask; outside is transparent
alpha[hard_mask == 0] = 0

# Erode slightly to remove thin halo
